        ...
        ajax: {
            ...
            results: function (data, page) {
                if (data.results != null) {
                    ...
                    return {results: [{id: '', text: 'No Match Found2'}]};
                }
            }
        }
    });
    ...
    if (localStorage.getItem('poitems')) {
        ...
        $.each(sortedItems, function () {
            ...
            var supplier = localStorage.getItem('posupplier'), belong = false;
				...
				weight = item.row.weight*base_quantity;
				...
				item.row.real_unit_cost = parseFloat(item.row.base_unit_cost);
				item_cost = parseFloat(item.row.base_unit_cost);
				unit_cost = parseFloat(item.row.base_unit_cost);
				
			if(item.product_wholesales && item.product_wholesales.length > 0) {
				item.product_wholesales.map(function(whole) {
				if(item_qty >= parseFloat(whole.quantity)) {
					item.row.real_unit_cost = parseFloat(whole.price);
					item_cost = parseFloat(whole.price);
					unit_cost = parseFloat(whole.price);
				}
				return null;
			});
			}	
			...
			//console.log(item.product_parent_qty);
			//alert(item.product_parent_qty[0].quantity);
			//$.each(item.options, function () {
			 /*if(base_quantity >item.product_parent_qty[0].quantity) {
				$('#row_' + row_no).addClass('danger');
				$('#add_pruchase, #edit_purchase').attr('disabled', true);
			 }else{
				 $('#row_' + row_no).removeClass('danger');
				 $('#add_pruchase, #edit_purchase').attr('disabled', false);
			 }*/
			// alert(item.product_parent_qty[0].quantity);
			//item.product_parent_qty[0].quantity
			if(item.product_parent_qty != null){
			if (typeof item.product_parent_qty[0].quantity !== "undefined") {
				if( $('#add_pruchase, #edit_purchase').attr('disabled', true)){
					 if(base_quantity > item.product_parent_qty[0].quantity) {
						$('#row_' + row_no).addClass('danger');
						$('#add_pruchase, #edit_purchase').attr('disabled', true);
						alert("จำนวนสินค้ามีไม่เพียงพอ กรุณาติดต่อผู้ที่คุณจะสังซื้อ");
					 }else{
						 $('#row_' + row_no).removeClass('danger');
						 $('#add_pruchase, #edit_purchase').attr('disabled', false);
					 }
					 ...
					 setTimeout(
						function(){
								if($('#row_' + row_no).hasClass("danger")==true){
									$('#add_pruchase, #edit_purchase').attr('disabled', true);
								}
						},
					1000);
