...
function loadItems() {
    ...
    if (localStorage.getItem('slitems')) {
        ...
        $.each(sortedItems, function () {
			...
			var product_award_points = item.row.award_points;
			...
			// if(site.settings.award_use == 1 || site.settings.award_use == 2){
				// product_award_points = product_award_points * parseInt(item_qty);
				// award_points = award_points + parseFloat(product_award_points);
				// tr_html += '<td class="text-right">'+ product_award_points +'</td>';
			// }
        ...
        var tfoot = '<tr id="tfoot" class="tfoot active"><th colspan="'+col+'">'+Ttotal+'</th><th class="text-center">' + formatQty(parseFloat(count) - 1) + '</th>';
		...
		// if(site.settings.award_use == 1 || site.settings.award_use == 2){
		// tfoot += '<th class="text-right">'+ award_points +'</th>';
		// }
		...
		$('#award_points').val(award_points);
        if (count > 1) {
            $('#slcustomer').select2("readonly", true);
            $('#slwarehouse').select2("readonly", true);
        }
		var shippingtitle = $("#shippingtitle option:first-child ").val();
		$("#shippingtitle").val(shippingtitle).change();
        set_page_focus();
    }
}
