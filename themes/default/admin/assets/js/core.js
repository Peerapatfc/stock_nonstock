...
$(function() {
    $('.datetime').datetimepicker({format: 'dd-mm-yyyy HH:ii:ss', fontAwesome: true, language: 'sma', weekStart: 1, todayBtn: 1, autoclose: 1, todayHighlight: 1, startView: 2, forceParse: 0});
	...
	$('.time').datetimepicker({ format: 'HH:ii p',autoclose: true, showMeridian: true,startView: 1,maxView: 1,fontAwesome: true, language: 'sma'});
...
$(document).on('ifChecked', '.checkftdeliveries', function(event) {
	id = $(this).attr('datack');

	
    $('#'+id + ' .checkftdeliveries').iCheck('check');
    $('#'+id + ' .multi-select').each(function() {
        $(this).iCheck('check');
    });
});
$(document).on('ifUnchecked', '.checkftdeliveries', function(event) {
	id = $(this).attr('datack');
	
    $('#'+id + ' .checkftdeliveries').iCheck('uncheck');
    $('#'+id + ' .multi-select').each(function() {
        $(this).iCheck('uncheck');
    });
});
...
function user_status(x) {
    var y = x.split("__");
    return y[0] == 1 ?
    '<span><span class="label label-success"><i class="fa fa-check"></i> '+lang['active']+'</span></span>' :
...
$(document).ready(function() {
    ...
    $('body').on('click', '.invoice_link td:not(:first-child, :nth-child(6), :nth-last-child(3), :last-child)', function() {
