    ...
    <div class="box-content">
        <div class="row">
            <div class="col-lg-12">
                ...
                <div class="col-md-6 col-md-offset-1">
                    <div class="standard">
                        <div>
                            ...
                            <div class="row"><div class="col-md-12"><div class="well">
                                <p><strong><?= lang("point_reward") ?></strong></p>
									<div class="controls" id="subcat_data"> <?php
										echo form_input('point_reward', ($product ? $product->award_points : ''), 'class="form-control" id="point_reward"  placeholder="' . lang("point_reward") . '"');
										?>
									</div>
                    ...
                    <div class="form-group standard">
                        <div class="form-group">
                            ...
                            <button type="button" class="btn btn-success btn-xs" id="addSupplier"><i class="fa fa-plus"></i>
                            </button>
                        </div>
				...
				<div class="form-group">
                        <div class="form-group">
                            กำหนดราคาส่งตามจำนวน
                            <button type="button" class="btn btn-success btn-xs" id="addWhosale"><i class="fa fa-plus"></i>
                            </button>
                        </div>
                        <?php
                        if(count($product_wholesales) > 0) {
                            foreach($product_wholesales as $whole) {
                                echo '<div class="row" style="margin-bottom: 5px;" id="rowWhosale'.$whole->id.$whole->id.'">';
                                    echo '<div class="col-xs-5">';
                                        echo '<input type="hidden" name="id_wholesale[]" value="'.$whole->id.'"/>';
                                        echo form_input('wholesale_quantity[]', (isset($whole->quantity) ? $whole->quantity : ""), 'class="form-control tip" placeholder="' . lang('wholesale_quantity') . '"');
                                    echo '</div>';
                                    echo '<div class="col-xs-5">';
                                       echo form_input('wholesale_price[]', (isset($whole->price) ? $whole->price : ""), 'class="form-control tip" placeholder="' . lang('wholesale_price') . '"');
                                    echo '</div>';
                                    echo '<div class="col-xs-2">';
                                        echo '<button class="btn btn-danger btn-sm" onclick="return delWhosale('.$whole->id.$whole->id.')" type="button"><i class="fa fa-minus"></i></button>';
                                    echo '</div>';
                                echo '</div>';
                            }
                        } else {
                        ?>
                            <div class="row" id="wholesale-con" style="margin-bottom: 5px;">
                                <div class="col-xs-5">
                                    <?= form_input('wholesale_quantity[]', (isset($_POST['wholesale_quantity']) ? $_POST['wholesale_quantity'] : ""), 'class="form-control tip" id="wholesale_quantity" placeholder="' . lang('wholesale_quantity') . '"'); ?>
                                </div>
                                <div class="col-xs-5">
                                    <?= form_input('wholesale_price[]', (isset($_POST['wholesale_price']) ? $_POST['wholesale_price'] : ""), 'class="form-control tip" id="wholesale_price" placeholder="' . lang('wholesale_price') . '"'); ?>
                                </div>
                            </div>    
                        <?php }
                        ?>
                        <div id="ex-wholesales"></div>
                    </div>
                ...
                <div class="col-md-12">
                    ...
                    <div class="form-group">
                        <?php echo form_submit('edit_product', $this->lang->line("edit_product"), 'class="btn btn-success"'); ?>
                    </div>
...
<script type="text/javascript">
    $(document).ready(function () {
	  ...
	  var wh = 2;
        $('#addWhosale').click(function () {
            // if (wh <= 5) {
                var html = '<div class="row" id="rowWhosale'+wh+'" style="margin-bottom: 5px;"> \
                        <div class="col-xs-5"> \
                            <input class="form-control tip" type="text" placeholder="<?=lang('wholesale_quantity')?>" name="wholesale_quantity[]"/> \
                        </div> \
                        <div class="col-xs-5"> \
                            <input class="form-control tip" type="text" placeholder="<?=lang('wholesale_price')?>" name="wholesale_price[]"/> \
                        </div> \
                        <div class="col-xs-2"> \
                            <button class="btn btn-danger btn-sm" onclick="return delWhosale('+wh+')" type="button"><i class="fa fa-minus"></i></button> \
                        </div> \
                    </div>';
                $('#ex-wholesales').append(html);
                wh++;
            // } else {
            //     bootbox.alert('<?= lang('max_reached') ?>');
            //     return false;
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-success" id="updateAttr"><?= lang('submit') ?></button>
            </div>
        </div>
    </div>
</div>
