                ...
                <div class="row">
					...
					<div class="col-md-12">
						<div class="col-md-6">
							<div class="form-group">
							<?php
								//	print_r($userbyparent);
								if($Settings->default_admin_user==$current_user_id->id):
									echo lang("sell_child_name", "sell_child_name"); 
									$up = [];
									$up[""] = "";
									foreach ($userbyparent as $ubp) {
										$up[$ubp->id] = $ubp->first_name." ".$ubp->last_name;
									}
								
									echo form_dropdown('sell_child_name', $up, '', 'class="form-control input-tip" required="required" id="sell_child_name"');
								endif;
							?>
							</div>
						</div>
                        ...
                        <?php if ($Settings->tax2) { ?>
							<?php if ($Owner || $Admin) {$_c = "";}else{$_c = "display:none";} ?>
                            <div style="<?php echo $_c; ?>" class="col-md-4">
                                <div class="form-group">
									<h2 style="margin-bottom:3px;"><i class="fa  fa-building-o"></i><?= lang("order_tax"); ?></h2>
                                    <?php
                                    $tr[""] = "";
					...
					<?php endif; ?>
                        ...
                        <div class="col-md-6">
								<div class="form-group">
									...
									echo form_dropdown('bank_to', $banks, '', 'class="form-control input-tip" required="required" id="slbank_to"'); ?>
								</div>
                        </div>	
                        <div class="col-md-6">
							<div class="row">
...
<script type="text/javascript">
    $(document).ready(function () {
                ...
                $.ajax({
                    ...
                    success: function (data) {
						...
						if(data[0].price == 'null'){
							...
							var gtotal = (total - order_discount) + shipping;
		...
		if(permission != 1){
			function slorder_type(){
				$("#slTable tbody tr").each(function(){
					setTimeout(function(){$("#slTable tbody tr").children("td:last-child").find("i").trigger('click'); }, 500);
				});
				...
				if(input_val == "stock"){
					...
					warehouse_id = '<?php echo $this->session->userdata('warehouse_id'); ?>';
					$("#slwarehouse").val(warehouse_id);
				}
			}
