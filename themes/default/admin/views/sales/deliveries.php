    ...
    $(document).ready(function () {
		...
		$("#myModal").on("hidden.bs.modal", function(){
			//window.location.href='<?php echo site_url('/admin/sales/deliveries'); ?>';
				$('#ready').dataTable().fnDraw();
				$('#packing').dataTable().fnDraw();
				$('#delivering').dataTable().fnDraw();
				$('#delivered').dataTable().fnDraw();
		});
		
	   jQuery('#page-delivered > div').each(function(){
		   var id = jQuery(this).attr('id');
		   var btn = jQuery(this).children('.buttons');
		   var htmlbtn = btn.html();
		   var databtn = btn.attr('data-btn');
		   jQuery('#'+ databtn + '_length').append(htmlbtn);
...
<?php #if ($Owner) { ?>
<?= admin_form_open('sales/delivery_actions', 'id="action-form" name="delivery_form"') ?>
<?php #} ?>
<div class="box">
        ...
        <div class="row">
            <div class="col-lg-12">
				...
				<ul id="dbTab" class="nav nav-tabs tab_delivering">
					...
					<?php endforeach;*/ ?>
                        <li class=""><a class="lid-ready" href="#tb_ready"><i class="fa fa-truck" aria-hidden="true"></i> <?= lang('ready') ?><span style="margin-left:3px;background-color: #d9534f;" class="badge badge-warning"><?php echo $countready; ?></span></a></li>
                        <li class=""><a class="lid-packing" href="#tb_packing"><i class="fa fa-dropbox" aria-hidden="true"></i>
<?= lang('packing') ?><span style="margin-left:3px;background-color: #f0ad4e;" class="badge"><?php echo $countpacking; ?></span></a></li>
                        <li class=""><a class="lid-delivering" href="#tb_delivering"><i class="fa fa-paper-plane" aria-hidden="true"></i>
<?= lang('delivering') ?><span style="margin-left:3px;background-color: #428bca;" class="badge"><?php echo $countdelivering; ?></span></a></li>
                        <li class=""><a class="lid-delivered" href="#tb_delivered"><i class="fa fa-hand-peace-o" aria-hidden="true"></i>
								...
								<table>
									<tr>
										<td>
											...
											echo form_dropdown('print_pdf', $print_pdf, '', 'class="form-control input-tip" required="required" id="print_pdf"'); ?>
										</td>
										<td>&nbsp;</td>
										<td>
        ...
        $(document).ready(function() {
            ...
            $(document).on('click', "#acprint_packing", function(e){
				...
				if(print_pdf == "Addressee"){
					...
					var action = "<?php echo site_url('admin/sales/print_shipping_label');?>";
					var form_action = "acprint_packing";
				}else if(print_pdf == "Agent"){
					var action = "<?php echo admin_url("sales/owner_receiver") ?>";
					var form_action = "sales_receiver";
				}else{
					var action = "<?php echo admin_url("sales/owner_receiver") ?>";
			...
			setInterval(function(){
				var c_ready = $('#ready').dataTable().fnGetData().length;
				var c_packing = $('#packing').dataTable().fnGetData().length;
				var c_delivering = $('#delivering').dataTable().fnGetData().length;
				var c_delivered = $('#delivered').dataTable().fnGetData().length;
						
				$('.lid-ready span.badge').text(c_ready);
				$('.lid-packing span.badge').text(c_packing);
				$('.lid-delivering span.badge').text(c_delivering);
				$('.lid-delivered span.badge').text(c_delivered);
				console.log(c_ready);
			},1200);
        });
    </script>
