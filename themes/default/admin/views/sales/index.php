...
<script>
    $(document).ready(function () {
        oTable = $('#SLData').dataTable({
            ...
            'sAjaxSource': '<?=admin_url('sales/getSales' . ($warehouse_id ? '/' . $warehouse_id : '') .'?v=1'. ($this->input->get('wisdom') ? '&wisdom='.$this->input->get('wisdom') : ''). ($this->input->get('attachment') ? '&attachment='.$this->input->get('attachment') : ''). ($this->input->get('delivery') ? '&delivery='.$this->input->get('delivery') : '')); ?>',
            ...
            'fnRowCallback': function (nRow, aData, iDisplayIndex) {
				...
				if(aData[9] == "due"){
					$('td:eq(9)', nRow).html('<div class="text-center"><span class="payment_status label label-danger"><i class="fa fa-pencil" aria-hidden="true"></i> <?php echo lang('due'); ?></span></div>');
				}
				...
				if(aData[11] != null){
					...
					}else if(aData[11] == "kerry"){
						text = '<?php echo lang("kerry"); ?>';
					}else if(aData[11] == "pickup"){
						text = '<?php echo lang("pickup"); ?>';
					}else{
						text =aData[11];
					}
					
					$('td:eq(11)', nRow).html( '<div class="text-center"><span">'+ text +'</span></div>' );
				}
				...
				if(aData[12] != null){
					$('td:eq(12)', nRow).addClass('popup-slip');
					$('td:eq(12)', nRow).html('<div class="text-center"><a href='+ uri +' data-toggle="modal" data-target="#myModal" class="tip" title="" data-original-title="<?php echo lang("attachment"); ?>"><i class="fa fa-file"></i></a></div>');
				}else{
					$('td:eq(12)', nRow).html('<div class="text-center text-muted"><i class="fa fa-file"></i></div>');
				}


				//วันเวลา
				var date = aData[13].split(" ");
				if('<?php echo date('Y-m-d'); ?>' == date[0]){ date[0] = "<?php echo lang('today'); ?>";}
				$('td:eq(13)', nRow).html('<span class="cdate"><i class="fa fa-calendar" aria-hidden="true"></i> '+ date[0] + '</span><span class="ctime"><i class="fa fa-clock-o" aria-hidden="true"></i> ' + date[1] + '</span>');

				
				$('td:eq(14)', nRow).addClass('custom-position');
				
				//is_delete
				if(aData[15] > 0){ nRow.className = "invoice_link repending id_delete danger";}
				
				
				//order_type
				if(aData[18] == "dropship"){
            ...
            {column_number: 3, filter_default_label: "[<?=lang('seller');?>]", filter_type: "text", data: []},
        ...
        // $("#order-type-list").change(function(){
            // oTable.fnFilter($(this).val(),18).fnDraw();
        // });

        var dropdownSelectOrderStatus = "<select id='order-status-list'><option value=''><?php echo lang('select_order_status'); ?></option><option value='wait'><?php echo lang('wait'); ?></option><option value='completed'><?php echo lang('completed'); ?></option></select>";
		...
		var add_sale = '<a style="margin-right:10px;" class="btn btn-danger" href="<?=admin_url('sales/add')?>"> <i class="fa fa-plus-circle"></i> <?=lang('Add_Order')?></a>';
		$("#SLData_length > label").append(add_sale);
...
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i
                class="fa fa-shopping-cart"></i><?=lang('sales') . ' (' . ($warehouse_id ? $warehouse->name : lang('all_warehouses')) . ')';?>
		</h2>
        ...
        <div class="box-icon">
            <ul class="btn-tasks">
                            ...
                            <?php
                            	foreach ($warehouses as $warehouse) {
									if($warehouse->id==$this->session->userdata('warehouse_id')){
										echo '<li><a href="' . admin_url('sales/' . $warehouse->id) . '"><i class="fa fa-building"></i>' . $warehouse->name . ''.lang("my_store").'</a></li>';
									}else{
                ...
                ?>
                <li class="dropdown">
                    <a data-toggle="dropdown" class="dropdown-toggle" href="#"><i class="icon fa fa-list-alt tip" data-placement="left" title="<?=lang("select_wisdom")?>"></i></a>
                    <ul class="dropdown-menu pull-right tasks-menus" role="menu" aria-labelledby="dLabel">
						<?php foreach($wisdom as $wisdom): ?>
                        <li<?= $this->input->get('wisdom') == $wisdom->id ? ' class="active"' : ''; ?>><a href="<?=admin_url('sales?wisdom='.$wisdom->id)?>"> <?= $wisdom->first_name." ".$wisdom->last_name;?></a></li>
						<?php endforeach; ?>
						
						<li<?= !$this->input->get('wisdom') ? ' class="active"' : ''; ?>><a href="<?=admin_url('sales')?>"> <?=lang('all_wisdom')?></a></li>
                    </ul>
                </li>
    ...
    <div class="box-content">
        <div class="row">
            <div class="col-lg-12">
				...
				<div id="form">
                    <?php echo admin_form_open("sales/sale_setting"); ?>
                    <div class="row">
						<?php #var_dump($sale_setting); ?>
                        <div class="col-sm-2">
                            <div class="form-group">
                                <label class="control-label" for="prefix"><?= lang("prefix"); ?></label>
                                <?php echo form_input('prefix', $sale_setting->prefix, 'class="form-control tip" id="prefix"'); ?>
                            </div>
                        </div>
                        ...
                        <div class="col-sm-2">
                            <div class="form-group">
                                ...
                                <?php echo form_input('digits', $sale_setting->digits, 'class="form-control tip" id="digits"'); ?>
                            </div>
                        </div>
                        ...
                        <div class="col-sm-2">
                            <div class="form-group">
                                <label class="control-label" for="start"><?= lang("start"); ?></label>
								<?php echo form_input('start', sprintf("%0".$sale_setting->digits."d", $sale_setting->start), 'class="form-control tip" id="start"'); ?>
                            </div>
                        </div>
                            <div class="col-sm-2">
                                <div class="form-group">
									<label class="control-label" for="max_count"><?= lang("max_count"); ?></label>
									<?php echo form_input('max_count', sprintf("%0".$sale_setting->digits."d", $sale_setting->max_count), 'class="form-control tip" id="max_count"'); ?>
                                </div>
                            </div>
							*/ ?>
                        <div class="col-sm-2">
                            <div class="form-group">
                                ...
                                <?php echo form_input('current', sprintf("%0".$sale_setting->digits."d", $sale_setting->current), 'class="form-control " id="current"'); ?>
                            </div>
                        </div>
                    </div>
                    <div class="form-group">
                ...
                <div class="table-responsive2">
                    <table id="SLData" class="table table-bordered table-hover table-striped">
                        ...
                        <tr>
							...
							<th class="" style="min-width:80px;"><?= lang("chanel"); ?></th>
                            <th class="threference_no"><?= lang("reference_no"); ?></th>
                            <th class="thbiller"><?= lang("seller"); ?></th>
...
<script type="text/javascript">
    $(document).ready(function () {
		$(document).on('click', '#print_order, #ship_soko', function(e) {
			e.preventDefault();
			//console.log("submit print_order");
