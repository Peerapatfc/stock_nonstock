    ...
    <div class="box-content">
        <div class="row">
            <div class="col-lg-12">
                ...
                <div class="print-only col-md-12">
                ...
                <div class="well well-sm">
                    ...
                    <div class="col-md-4 border-right">
                        ...
                        <div class="col-xs-10">
                            ...
                            echo $customer->address . "<br>" . $customer->country;
                    ...
                    <div class="col-md-4 border-right">
                        ...
                        <div class="col-xs-10">
                            <h2 class=""><?= $created_by->first_name.' '.$created_by->last_name; ?></h2>
							

							
                            <?php // $user_byemail->company ? "" : "Attn: " . $user_byemail->first_name; ?>

                            <?php
                            echo $user_byemail->address . "<br>" . $user_byemail->city . " " . $user_byemail->postal_code . " " . $user_byemail->state . "<br>" . $user_byemail->country;

                            echo "<p>";

                            if ($user_byemail->vat_no != "-" && $user_byemail->vat_no != "") {
                                echo "<br>" . lang("vat_no") . ": " . $user_byemail->vat_no;
                            }
                            ...
                            echo "</p>";
                            echo lang("tel") . ": " . $created_by->phone . "<br>" . lang("email") . ": " . $created_by->email;
                            ?>
                        </div>
                        <div class="clearfix"></div>
                ...
                <div class="col-md-5">
                    ...
                    <div class="col-xs-10 view-information-order">
                        <h2 class=""><em class="text-title"><?= lang("ref"); ?></em>: <?= $inv->reference_no; ?></h2>
						
						
                        <?php if (!empty($inv->return_sale_ref)) {
                            echo '<p><em class="text-title">'.lang("return_ref").'</em>: '.$inv->return_sale_ref;
                            if ($inv->return_id) {
                                echo ' <a data-target="#myModal2" data-toggle="modal" href="'.admin_url('sales/modal_view/'.$inv->return_id).'"><i class="fa fa-external-link no-print"></i></a><br>';
                            } else {
                                echo '</p>';
                        ...
                        <p style="font-weight:bold;"><em class="text-title"><?= lang("date"); ?></em>: <?= $this->sma->hrld($inv->date); ?></p>

                        <p style="font-weight:bold;"><em class="text-title"><?= lang("sale_status"); ?></em>: <?= lang($inv->sale_status); ?></p>

                        <p style="font-weight:bold;"><em class="text-title"><?= lang("payment_status"); ?></em>: <?= lang($inv->payment_status); ?></p>
						
						<?php if($inv->delivery_type): ?>
                        <p style="font-weight:bold;"><em class="text-title"><?= lang("delivery_type"); ?></em>: <?= lang($inv->delivery_type); ?></p>
						<?php endif; ?>
						
						<?php if ($inv->order_type || $inv->order_type != "") { ?>
						<?php 
								if($inv->order_type == "dropship"):
									$_icon = '<span  class="label" style="padding: 3px 8px;font-size: 100%;background-color:#FF7600;"><i class="fa fa-paper-plane" aria-hidden="true"></i> '.lang($inv->order_type).'</span>';
								else:
									$_icon = '<span class="label" style="padding: 3px 8px;font-size: 100%;background-color:#123EAB;"><i class="fa fa-cubes" aria-hidden="true"></i> '.lang($inv->order_type).'</span>';
								endif;
						?>
							<p style="font-weight:bold;"><em class="text-title"><?= lang("order_type"); ?></em>: <?= $_icon; ?></p>
						<?php } ?>
						
						<?php if($inv->is_deliveries){ ?>
							<p style="font-weight:bold;"><em class="text-title"><?= lang("tracking"); ?></em>: <?php echo  !is_null($deliveries->tracking) ? '<span class="panel-success"><i class="fa fa-truck" aria-hidden="true"></i> '.$deliveries->tracking.'</span>' : '<span class="panel-danger"><i class="fa fa-truck" aria-hidden="true"></i> '.lang("Pending_tracking").'</span>'; ?></p>
						<?php }else{ ?>
							<p style="font-weight:bold;"><em class="text-title"><?= lang("tracking"); ?></em>: <?php echo  !is_null($deliveries->tracking) ? '<span class="panel-success"><i class="fa fa-truck" aria-hidden="true"></i> '.$deliveries->tracking.'</span>' : '<span class="panel-danger"><i class="fa fa-truck" aria-hidden="true"></i> '.lang("Pending_order").'</span>'; ?></p>
						<?php } ?>
                    ...
                    <table class="table table-bordered table-hover table-striped print-table order-table">
						...
						// echo "<br/>".$row->product_name;
                            ...
                            if (isset($tax_summary[$row->tax_code])) {
                                ...
                                $tax_summary[$row->tax_code]['tax'] = $row->item_tax;
                                $tax_summary[$row->tax_code]['amt'] = ($row->unit_quantity * $row->net_unit_price) - $row->item_discount;
                                $tax_summary[$row->tax_code]['name'] = $row->product_name;
                                $tax_summary[$row->tax_code]['code'] = $row->product_code;
                ...
                <div class="row">
                    <div class="col-md-6">
                        ...
                        if ($inv->order_status_history || $inv->order_status_history != "") { ?>
                            <div class="well well-sm">
								<p class="bold"><?= lang("Payment Confirm"); ?>:</p>
								<p> <?= $this->sma->decode_html($inv->order_status_history); ?></p>
								<?php $referer = admin_url('welcome/download/'.$this->sma->decode_html($inv->attachment)); ?>
								<div><?= '<img src="'.$referer.'" alt="" class="" style="width: 450px; max-width:100%;">'; ?></div>
                            </div>
                        <?php
                        }
                        ...
                        <?php if ($Settings->award_use > 0) { ?>
                        <div class="col-xs-12 col-sm-10 col-md-8 col-lg-6">
                            <div class="well well-sm">
                                <?=
                                '<p>'.lang('this_sale').': '.$point->points_current
                                .'<br>'.
                                lang('your_award_points').': '. $totalpoint . '</p>';?>
                            </div>
                        </div>
                        <?php } ?>
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        if ($Settings->invoice_view == 1) {
                            if (!empty($tax_summary)) {
                                ...
                                echo '<div class="table-responsive"><table class="table table-bordered table-condensed"><thead><tr><th>' . lang('name') . '</th><th>' . lang('code') . '</th><th>' . lang('qty') . '</th><th>' . lang('tax_excl') . '</th><th>' . lang('tax_amt') . '</th></tr></td><tbody>';
                        ...
                        <?php if ($paypal->active == "1" && $inv->grand_total != "0.00") {
                            ...
                            <div class="hidden col-md-6 text-center">
                                <form action="https://www.paypal.com/cgi-bin/webscr" method="post">
                                    ...
                                    <button type="submit" name="submit" class="btn btn-success btn-lg btn-block"><i
        ...
        <?php if (!$Supplier || !$Customer) { ?>
            <div class="buttons">
                <div class="btn-group btn-group-justified">
                    ...
                    <?php } ?>
                    <div class="btn-group">
                        <a href="<?= admin_url('sales/payments/' . $inv->id) ?>" data-toggle="modal" data-target="#myModal" class="tip btn  btn-success tip" title="<?= lang('view_payments') ?>">
                            <i class="fa fa-money"></i> <span class="hidden-sm hidden-xs"><?= lang('view_payments') ?></span>
                        </a>
                    </div>
                    <div class="btn-group">
                        <a href="<?= admin_url('sales/add_payment/' . $inv->id) ?>" data-toggle="modal" data-target="#myModal" class="tip btn  btn-success tip" title="<?= lang('add_payment') ?>">
                            <i class="fa fa-money"></i> <span class="hidden-sm hidden-xs"><?= lang('add_payment') ?></span>
                        </a>
                    </div>
                    <div class="btn-group">
                        <a href="<?= admin_url('sales/email/' . $inv->id) ?>" data-toggle="modal" data-target="#myModal" class="tip btn  btn-success tip" title="<?= lang('email') ?>">
                            <i class="fa fa-envelope-o"></i> <span class="hidden-sm hidden-xs"><?= lang('email') ?></span>
                        </a>
                    </div>
                    <div class="btn-group">
                        <a href="<?= admin_url('sales/pdf/' . $inv->id) ?>" class="tip btn  btn-success" title="<?= lang('download_pdf') ?>">
                            <i class="fa fa-download"></i> <span class="hidden-sm hidden-xs"><?= lang('pdf') ?></span>
                        </a>
                    </div>
                    <?php if ( ! $inv->sale_id) { ?>
                    <div class="btn-group">
                        <a href="<?= admin_url('sales/add_delivery/' . $inv->id) ?>" data-toggle="modal" data-target="#myModal" class="tip btn  btn-success tip" title="<?= lang('add_delivery') ?>">
                    ...
                    <!--<div class="btn-group"><a href="<?= admin_url('sales/excel/' . $inv->id) ?>" class="tip btn btn-success"  title="<?= lang('download_excel') ?>"><i class="fa fa-download"></i> <?= lang('excel') ?></a></div>-->
                </div>
            </div>
        <?php } ?>
    </div>
