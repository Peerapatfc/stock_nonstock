    ...
    $(document).ready(function () {
        ...
        oTable = $('#UsrTable').dataTable({
			...
			'fnRowCallback': function (nRow, aData, iDisplayIndex) {
				...
				//console.log(aData);
				/* check default dealer */
				/*var check_dealer = "<?php echo $this->site->getUser();?>";
				if (aData[0] == 'wait')
				{
				...
				if(aData[1] == ""){
					var txt = "<?php echo lang('not_working'); ?>";
					$('td:eq(1)', nRow).html('<div style="color: #d9534f;" class="text-muted ">'+txt+'</div>');
				}
				if(aData[8] == "1"){
					var txt = "<?php echo lang('default_dealer'); ?>";
					$('td:eq(6)', nRow).html('<span class="label label-success">'+txt+'</span>');
				}else{
					$('td:eq(6)', nRow).html('');
				}
				...
				if(aData[9] == "waiting_approve"){
					var txt = "<?php echo lang('waiting_approve'); ?>";
					//$('td:eq(8)', nRow).html('<span class="label label-warning">'+txt+'</span>');
				}else if(aData[9] == "not_approve"){
					var txt = "<?php echo lang('not_approve'); ?>";
					$('td:eq(7)', nRow).css({"text-align":"center"}).html('<span class="text-danger"><i class="fa fa-times" aria-hidden="true"></i> '+txt+'</span>');
				}else if(aData[9] == "approve" || aData[6] != "waiting"){ //
					var txt = "<?php echo lang('approved_successfully'); ?>";
					$('td:eq(7)', nRow).css({"text-align":"center"}).html('<span class="text-success"><i class="fa fa-check" aria-hidden="true"></i> '+txt+'</span>');
				}else{
					//$('td:eq(8)', nRow).html('');
				}
                ...
                "bSortable": false,
                "mRender": checkbox
            }, null, null, null,  null, {"bVisible": false}, null, 
			{"mRender": user_status, "bVisible": false},
			null,{"bVisible": false},null,null]

        }).fnSetFilteringDelay().dtFilter([
			{column_number: 1, filter_default_label: "[<?=lang('seller_id');?>]", filter_type: "text", data: []},
                ...
                <div class="table-responsive">
                    <table id="UsrTable" cellpadding="0" cellspacing="0" border="0"
                        ...
                        <tr>
							...
							<th class="col-xs-2"><?php echo lang('seller_id'); ?></th>
							...
							<th style="width:80px;"><?php echo lang('default_dealer'); ?></th>
							<th style="width:80px;"><?php echo lang('refer_status'); ?></th>
							<th style="width:80px;"><?php echo lang('refer_status'); ?></th>
