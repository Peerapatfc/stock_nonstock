...
<script type="text/javascript" charset="UTF-8">var oTable = '', r_u_sure = "<?=lang('r_u_sure')?>";
	...
	//window.location.href = "http://demo.smith.in.th/admin/sales";
	 // $('#salesearch').submit(function(){
		// window.location.href = "http://demo.smith.in.th/admin/sales";
		// exit();
	 // });
	 
	
	// console.log('<?php echo $this->session->userdata('user_id'); ?>');

        // $("#salesearch").autocomplete({
            // source: function (request, response) {
                // $.ajax({
                    // type: 'get',
                    // url: '<?= admin_url('sales/search'); ?>',
                    // dataType: "json",
                    // data: {
                        // term: request.term
                    // },
            ...
            // response: function (event, ui) {
                ...
                // if ($(this).val().length >= 16 && ui.content[0].id == 0) {
                    // bootbox.alert('<?= lang('no_found') ?>', function () {
                        // $('#salesearch').focus();
                    // });
                    // $(this).removeClass('ui-autocomplete-loading');
                    // $(this).removeClass('ui-autocomplete-loading');
