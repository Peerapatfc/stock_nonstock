    ...
    $(document).ready(function () {
        oTable = $('#SLData').dataTable({
            ...
            'fnRowCallback': function (nRow, aData, iDisplayIndex) {
				...
				/* start wow */
				var ck = '<?php echo $Owner; ?>', uri = '<?php echo admin_url("sales/popup/") ?>' + aData[11];
				if(aData[9] == "pending"){
					$('td:eq(5)', nRow).addClass('asset-locked disable');
				}
				
				if(ck != 1){ nRow.className = "disable";}
				if(aData[13] > 0){nRow.className = "disable_product_link danger";}
				
				if (aData[5] == 'wait')
				{
					$('td:eq(5)', nRow).html( '<div class="text-center"><span class="row_status label label-warning"><?php echo lang('wait'); ?></span></div>' );
				}
				...
				if(aData[10] != null){
					...
					if (aData[12] == '1')
					{
						$('td:eq(10)', nRow).html( '<div class="text-center"><span class="row_delivery label label-success"> </span></div>' );
					}else{
						$('td:eq(10)', nRow).html( '<div class="text-center"><span class="row_delivery label label-danger"> </span></div>' );
					}
				}
				/* end wow */
                return nRow;
