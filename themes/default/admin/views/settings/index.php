...
<script>
    $(document).ready(function () {
        ...
        var OrderNumberButton = function (context) {
          var ui = $.summernote.ui;

          // create button
          var button = ui.button({
            contents: 'Order Number',
            tooltip: 'insert order number',
            click: function () {
              context.invoke('editor.insertText', '{order_number}');
            }
          });
        ...
        var TrackingNumberButton = function (context) {
          var ui = $.summernote.ui;

          // create button
          var button = ui.button({
            contents: 'Tracking Number',
            tooltip: 'insert tracking number',
            click: function () {
              context.invoke('editor.insertText', '{tracking_number}');
            }
          });
        ...
        // if($(".sms-template").val()==''){
        //     $(".sms-template").val("เลขที่สั่งซื้อ {order_number} หมายเลขส่งสินค้าของคุณคือ {tracking_number}");
        // }
        
        $('.sms-template').summernote({
            toolbar: [
                ['mybutton', ['orderNumber']],
                ['mybutton', ['trackingNumber']],
            ],

            buttons: {
                orderNumber: OrderNumberButton,
                trackingNumber: TrackingNumberButton,
            },
            minHeight: 70,
            placeholder: 'Please insert sms template before send sms',
            callbacks: {
                onInit: function() {
                    setTimeout(function(){
                        $(".redactor_box").remove(); // remove auto render textarea by redactor
                    },1000);
                },
                ...
                onKeyup: function (e) {
                    var t = e.currentTarget.innerText;
                    $('#maxContentPost').text(67 - t.trim().length);
                },
                onPaste: function (e) {
                    var t = e.currentTarget.innerText;
                    ...
                    document.execCommand('insertText', false, all.trim().substring(0, 67));
                    $('#maxContentPost').text(67 - t.length);
                }
            }
        });

        // end sms template script
    });
</script>
<div class="box">
    ...
    <div class="box-content box_system_settings">
        <div class="row">
            <div class="col-lg-12">
                ...
                <div class="row">
                    <div class="col-lg-12">
                        ...
                        <fieldset class="scheduler-border system_site_configuration">
				...
				<div class="col-md-4">
                        <div class="form-group">
                            <label class="control-label"
                                   for="default_admin_user"><?= lang("default_admin_user"); ?></label>

                            <?php
                                foreach ($users as $user) {
                                    $us[$user->id] = $user->username . ' (' . $user->email . ')';
                                }
                                echo form_dropdown('default_admin_user', $us, $Settings->default_admin_user, 'class="form-control tip" id="default_admin_user" required="required" style="width:100%;"');
                                ?>
					...
					<div class="col-md-4">
                        <div class="form-group">
                            <?= lang("default_biller_sync", "biller_sync"); ?>
                            <?php
                            $bl[""] = "";
                            foreach ($billers as $biller) {
                                $bl[$biller->id] = $biller->company != '-' ? $biller->company : $biller->name;
                            }
                            echo form_dropdown('biller_sync', $bl, (isset($_POST['biller_sync']) ? $_POST['biller_sync'] : $Settings->default_biller_sync), 'id="biller_sync" data-placeholder="' . lang("select") . ' ' . lang("biller_sync") . '" required="required" class="form-control input-tip select" style="width:100%;"');
                            ?>
                        </div>
                    </div>
					<div class="col-md-4">
                        <div class="form-group">
							<label class="control-label" for="web_sync_url"><?= lang("web_sync_url"); ?></label>
                            <?php
                            echo form_input('web_sync_url', $Settings->web_sync_url, 'class="form-control tip" id="web_sync_url"');
						   ?>
                        </div>
                    </div>
					<div class="col-md-4">
                        <div class="form-group">
							<label class="control-label" for="api_username"><?= lang("api_username"); ?></label>
                            <?php
                            echo form_input('api_username', $Settings->api_username, 'class="form-control tip" id="api_username"');
						   ?>
                        </div>
                    </div>
					<div class="col-md-4">
                        <div class="form-group">
							<label class="control-label" for="api_password"><?= lang("api_password"); ?></label>
                            <?php
                            echo form_password('api_password', $Settings->api_password, 'class="form-control tip" id="api_password"');
						   ?>
                        </div>
                    </div>
                    <?php if (SHOP) { ?>
                    ...
                    <fieldset class="scheduler-border system_products">
                    ...
                    <fieldset class="scheduler-border system_sales">
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                <label class="control-label"
                                    ...
                                    <?php
                                    $ref = array(1 => lang('prefix_year_no'), 2 => lang('prefix_month_year_no'), 3 => lang('sequence_number'), 5 => lang('custom_number'), 4 => lang('random_number'));
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                <label class="control-label" for="default_shipping"><?= lang("default_shipping"); ?></label>
									<?php 
									$default_shipping = array('price' => lang('ship_for_price'), 'item' => lang('ship_for_item'), 'weight' => lang('ship_for_weight'));
									echo form_dropdown('default_shipping', $default_shipping, $Settings->default_shipping, 'class="form-control input-tip" required="required" id="default_shipping"'); ?>
                            </div>
                        </div>
                        ...
                        <div class="col-md-4">
                            ...
                            foreach ($suppliers as $supplier) {
                                $sp[$supplier->id] = $supplier->company != '-' ? $supplier->company : $supplier->name;
                            }

                            echo form_dropdown('supplier', $sp, $Settings->supplier, 'id="supplier" data-placeholder="' . lang("select") . ' ' . lang("supplier") . '" required="required" class="form-control input-tip select" style="width:100%;"');
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                <label class="control-label" for="addresses"><?= lang("addresses"); ?></label>
								<?= form_input('addresses', $Settings->addresses, 'class="form-control tip" id="addresses"'); ?>
                            </div>
                        </div>
						
                        <div class="col-md-4">
                            <div class="form-group">
                                <label class="control-label" for="billing_addresses"><?= lang("billing_addresses"); ?></label>
								<?= form_input('billing_addr', $Settings->billing_addr, 'class="form-control tip" id="billing_addresses"'); ?>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
                                <label class="control-label" for="taxno"><?= lang("taxno"); ?></label>
								<?= form_input('taxno', $Settings->taxno, 'class="form-control tip" id="taxno"'); ?>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
                                <label class="control-label" for="billing_phone"><?= lang("billing_phone"); ?></label>
								<?= form_input('billing_phone', $Settings->billing_phone, 'class="form-control tip" id="billing_phone"'); ?>
                            </div>
                        </div>
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                <label class="control-label" for="phone"><?= lang("phone"); ?></label>
									<?= form_input('phone', $Settings->phone, 'class="form-control tip" id="phone"'); ?>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
                                <label class="control-label" for="company_name"><?= lang("company_name"); ?></label>
								<?= form_input('company_name', $Settings->company_name, 'class="form-control tip" id="phone"'); ?>
                            </div>
                        </div>
                    ...
                    <fieldset class="scheduler-border system_prefix">
                    ...
                    <fieldset class="scheduler-border system_money_and_number_format">
                    ...
                    <fieldset class="scheduler-border system_email">
                        <legend class="scheduler-border"><?= lang('sms_template') ?></legend>
                        <div id="sms-template-input">
                            <!-- <textarea name="sms_template" class="skip sms-template">เลขที่สั่งซื้อ {order_number} หมายเลขส่งสินค้าของคุณคือ {tracking_number}</textarea> -->
                            <?= form_textarea('sms_template', $Settings->sms_template, 'class="skip sms-template"') ?>
                        </div>
                    ...
                    <fieldset class="scheduler-border system_award_points">
                            ...
                            <div class="form-group ">
                                ...
                                <div class="row">
                                    <div class="col-sm-4 col-xs-6">
										<?php 
											$award_use = array('0' => lang('disable'), '1' => lang('total_sale_order') , '2' => lang('product_point'));
                                            echo form_dropdown('award_use', $award_use, $Settings->award_use, 'class="form-control tip" id="award_use"');
										?>
                                    </div>
                                </div>
                            </div>
					   ...
					   <fieldset class="scheduler-border system_award_points">
                        <legend class="scheduler-border"><?= lang('wallet') ?></legend>
                        <div class="col-md-12 ">
                            <div class="form-group">
                                <div class="row">
                                    <div class="col-sm-4 col-xs-6">
                                        <?= lang('wallet_use'); ?><br>
										<?php 
											$wallet_use = array('0' => lang('no'), '1' => lang('yes'));
                                            echo form_dropdown('wallet_use', $wallet_use, $Settings->wallet_use, 'class="form-control tip" id="wallet_use"');
										?>
                                    </div>
                                </div>
                            </div>
            ...
            <div class="col-md-12">
                <div class="form-group">
                        ...
                        <?= form_submit('update_settings', lang("update_settings"), 'class="btn btn-success"'); ?>
    ...
    <div class="alert alert-info btn btn-margin-top" role="alert"><p>
        <strong>Cron Job:</strong> <code>0 1 * * * wget -qO- <?= admin_url('cron/run'); ?> &gt;/dev/null 2&gt;&amp;1</code> to run at 1:00 AM daily. For local installation, you can run cron job manually at any time.
        <?php if (!DEMO) { ?>
            <a class="btn btn-primary btn-xs pull-right" target="_blank" href="<?= admin_url('cron/run'); ?>">Run cron job now</a>
        <?php } ?>
...
<script type="text/javascript" charset="utf-8">
    $(document).ready(function () {
        $('.no').slideUp();
        $('#award_use').change(function (event) {
            var award_use = $(this).val();
            if (award_use == 0 || award_use == 2) {
                $('.no').slideUp("1200");
            } else {
                $('.no').slideDown("1200");
            }
