...
class system_settings extends MY_Controller
{
    ...
    function getCommission()
    {
        ...
        $this->load->library('datatables');
        $this->datatables
            ->select("{$this->db->dbprefix('commission_matrixrate')}.pk as id, {$this->db->dbprefix('commission_matrixrate')}.condition_from_value, {$this->db->dbprefix('commission_matrixrate')}.condition_to_value,  {$this->db->dbprefix('commission_matrixrate')}.price, {$this->db->dbprefix('commission_matrixrate')}.type", FALSE)
    ...
    function edit_commission($id = NULL)
    {
		...
		$details = $this->settings_model->getCommissionByID($id);
        if ($this->form_validation->run() == true) {
            $data = array(
                'condition_from_value' => $this->input->post('condition_from_value'),
                'condition_to_value' => $this->input->post('condition_to_value'),
