                ...
                <div class="row">
                    <div class="col-md-12">
                        ...
                        ?>
                            ...
                            <div class="form-group">
                                <?= form_submit('update', lang('update'), 'class="btn btn-success"'); ?>
                            </div>
                        <?php
                            echo form_close();
                        } else {
                            if (!empty($updates->data->updates)) {
                                ...
                                foreach ($updates->data->updates as $update) {
                                    ...
                                    if ($c == 1 && !empty($update->filename)) {
                                        echo anchor('admin/system_settings/install_update/' . substr($update->filename, 0, -4) . '/' . (!empty($update->mversion) ? $update->mversion : 0) . '/' . $update->version, '<i class="fa fa-download"></i> ' . lang('install'), 'class="btn btn-success pull-right"') . ' ';
                                    }
