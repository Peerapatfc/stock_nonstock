			...
			<div class="container-reg">
				<div class="row">
					<div id="content" class="col-sm-12 full">
						<div class="header">
							<?php //register_logo.png ?>
							<img alt="" src="<?= base_url('assets/images/logo.png'); ?>" class="mini_logo img-rounded">
						</div>
						...
						<div class="row">
							<div class="login-box">
								...
								<div class="form-reg">
									<div class="row">
										<div class="col-md-12">
											...
											<h2 class="title"><?= $this->lang->line('user_register_form') ?></h2>
											</div>
										</div>
									</div>
										...
										<div class="col-md-12">
											<div class="col-content">
												...
												<div class="form-group">
													<?php echo lang('parent_name', 'parent_name'); ?>
													<h4 style="background-color: rgba(28,175,154,.2);padding: 5px 8px;margin-top: 0;"><?php echo $parent_name; ?></h4>
												</div>

												<div class="form-group">
												<?php echo lang('advisor', 'advisor'); ?>
													<div class="controls">
													<?php echo form_input('advisor', '', 'class="form-control" id="searchadvisor" autocomplete="off" required="required"'); ?>
													<span class="icon-search"><i class="glyphicon glyphicon-search"></i></span>
													<ul class="list-group" id="advisorresult"></ul>
													</div>
													
													<?php echo form_input('id_advisor', '', 'class="form-control hidden" id="id_advisor" autocomplete="off" required="required"'); ?>
												...
												<?php echo lang('first_name', 'first_name'); ?>
													<div class="controls">
													<?php echo form_input('first_name', '', 'class="form-control" id="first_name" required="required" pattern=".{3,99}"'); ?>
													</div>
												</div>
												...
												<div  class="form-group">
													...
													<?php echo form_input('district', '', 'autocomplete="off" class="form-control"  id="district" '); ?>
													</div>
													</div>
													<div style="margin-bottom: 5px;" class="col-md-60">
													<div class="form-group03">
													<?= lang("amphoe", "amphoe"); ?>
													<?php echo form_input('amphoe', '', ' autocomplete="off" class="form-control"   id="amphoe" '); ?>
													</div>
													</div>
													<div class="col-md-60">
													<div style="margin-bottom: 5px;" class="form-group03">
													<?= lang("province", "province"); ?>
													<?php echo form_input('province', '', 'autocomplete="off" class="form-control"  id="province" '); ?>
													</div>
													</div>
													<div class="col-md-60">
													<div style="margin-bottom: 5px;" class="form-group03">
													<?= lang("zipcode", "zipcode"); ?>
													<?php echo form_input('zipcode', '', 'autocomplete="off" class="form-control" id="zipcode" '); ?>
													</div>
													</div>
												...
												<div class="form-group">
												<table class="or user_infomation">
													<tbody><tr>
														<td><div class="pslogin-border"></div></td>
														<td class="pslogin-bordertext w25"><?php echo lang('user_infomation'); ?></td>
														<td><div class="pslogin-border"></div></td>
													</tr></tbody>
												</table>
												</div>
											...
											<div class="col-content">
												<div class="form-group">
													...
													<div style="" class="col-md-12">
														<label class="checkbox" for="iagree"> <input type="checkbox" name="iagree" value="1" id="iagree" required="required" checked="checked" /> <?= lang('I_agree_to_the') ?><a class="i_agree"  href="javascript:void(0)"><?= lang('terms_and_conditions_of_agent'); ?></a></label>
														<?php @$this->load->view('website/includes/term') ?>
													</div>
													<div class="clearfix"></div>
								...
								<?php 
									...
									echo "<p style='text-aligntext-a;text-align: center;color: red;font-size: 16px;'>".lang("please_contact_the_facilitator_or_the_parent_team_to_request_a_representative_subscription_link.")."</p>";
									echo "<style>.mini_logo.img-rounded {max-width: 185px;}.register .bblue {background: none;} #content {padding: 0 0 5%;background: rgba(236,128,177,.2);margin-top: 15%;}</style>";
								endif; 
								?>
								
								<div class="clearfix"></div>
								<p class="legal">© 2017 All Rights Reserved. Proudly created with <a title="smith ระบบตัวแทน" href="https://www.atcreative.co.th/smith/" target="__blank"><img style="width: 16px;" src="<?= $assets ?>images/copyright_pink.png"></a></p>
							</div>
						</div>
					</div>
				</div>
...
<script src="<?= $assets ?>js/jquery.Thailand.js/jquery.inputmask.bundle.js"></script>
...
<script type="text/javascript">
   ...
   $(document).ready(function(){
	   setTimeout(function(){
		...
		var advisor;
		$.getJSON('<?=base_url();?>admin/auth/getuserall', function(data) {
		advisor = data;
		//console.log(advisor);
			  ...
			  $('#searchadvisor').keyup(function(){
				...
				var expression = new RegExp(searchField, "i");
				  $.each(advisor, function(key, value){
					  if (value.first_name.search(expression) != -1 || value.last_name.search(expression) != -1 || value.seller_id.search(expression) != -1)
					  {
						if (value.img == null) {
							$('#advisorresult').append('<li data_id="'+value.id+'" class="list-group-item link-class"><table><tr><td rowspan="2"><img src="<?=base_url();?>assets/images/'+value.gender+'.png" height="40" width="40" class="img-thumbnail" /></td><td rowspan="2">&nbsp</td><td class="_name">'+value.first_name+ " " +value.last_name+ '</td></tr><tr><td><span class="text-muted">'+ " " +value.seller_id+'</span><span style="visibility:hidden">'+ " " +value.id+'</span></td></tr></table></li>');
						}
						else{
							$('#advisorresult').append('<li data_id="'+value.id+'" class="list-group-item link-class"><table><tr><td rowspan="2"><img src="<?=$assets;?>agent-card/assets/images/imgup/tmp/'+value.img+'" height="40" width="40" class="img-thumbnail" /></td><td rowspan="2">&nbsp</td><td class="_name">'+value.first_name+ " " +value.last_name+ '</td></tr><tr><td><span class="text-muted">'+ " " +value.seller_id+'</span><span style="visibility:hidden">'+ " " +value.id+'</span> </td></tr></table></li>');
						}
					  }
				  });
			});
		...
		$('#advisorresult').on('click', 'li', function() {
			var click_text = $(this).find("._name").text();
			$('#searchadvisor').val(click_text);
			var data_id = $(this).attr("data_id");
			$('#id_advisor').val(data_id);
			$("#advisorresult").html('');
		});
