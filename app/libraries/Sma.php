...
#$this->sma->insertdb_award_points($data['grand_total'], $data['created_by'], $data['date'], $data['reference_no']);
    public function insertdb_award_points($total, $user = NULL, $date_insertion, $reference_no, $scope = null)
    {
        ...
        if ($user && !empty($this->Settings->each_sale) && !$this->Customer && $total >= $this->Settings->each_sale) {
            ...
            $this->db->insert('award_points', $data);
        }
        return true;
    }
    ...
    public function update_award_points($total, $customer, $user = NULL, $scope = null)
    {
        if (!empty($this->Settings->each_spent) && $total >= $this->Settings->each_spent) {
           ...
           //$total_points = $scope ? $company->award_points - $total : $company->award_points + $total;
           $this->db->update('companies', array('award_points' => $total), array('id' => $customer));
           //$this->db->update('companies', array('award_points' => $total), array('id' => $customer));
        }
        ...
        if ($user && !empty($this->Settings->each_sale) && !$this->Customer && $total >= $this->Settings->each_sale) {
            ...
            //$points = $total;
            //$total_points = $scope ? $staff->award_points - $points : $staff->award_points + $points;
            $this->db->update('users', array('award_points' => $total), array('id' => $user));
        }
        return true;
    }
