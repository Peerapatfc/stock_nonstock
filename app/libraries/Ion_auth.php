...
class Ion_auth
{
        ...
        if ($this->auth_model->forgotten_password($identity)) {   //changed
            ...
            if ($user) {
                ...
                if (!$this->config->item('use_ci_email', 'ion_auth')) {
					...
					$this->forgotten_password_complete($code);
        ...
        if ($new_password) {
            ...
            if (!$this->config->item('use_ci_email', 'ion_auth')) {
				...
				if(strlen(trim($profile->email)) > 0 ){
					$val_email= $profile->email;
				}else{
					$val_email= $profile->username;
				}
				
                $parse_data = array(
                    'email' => $val_email,
                    'password' => $new_password,
                    'site_link' => "http://bit.ly/BC99login",
                    );
                $msg = file_get_contents('./themes/' . $this->Settings->theme . '/admin/views/email_templates/sms.html');
                //$message = $this->parser->parse_string($msg, $parse_data);
                ...
                if(strlen(trim($phone)) > 0){
                    ...
                    $from = "020000000";
                    $to = $phone;
                    $message = strtr($msg, $replace); 	

					$sms_limit = $this->Settings->sms_limit;
					$sms_count = $this->Settings->sms_count;
					...
					if($sms_limit >= $sms_count){
						...
						$this->object_settings_model->updateSetting($data);
						$sms = new thsms();
						$sms_response = $sms->send($from, $to, $message);

						if($sms_response[0] === false){
							$this->session->set_flashdata('error', sprintf(lang("fail_to_send_%s_sms!"), $phone));
							redirect($_SERVER["HTTP_REFERER"]);
						}else{
							$this->session->set_flashdata('message', sprintf(lang("send_sms_new_password_to_phone_%s"), $phone));
							redirect($_SERVER["HTTP_REFERER"]);
						}
					}else{
						$this->session->set_flashdata('error', lang('sms_max_limit'));
					}
                }else{
					$this->session->set_flashdata('error', lang('fail_to_send_sms!.'));
					redirect($_SERVER["HTTP_REFERER"]);
				}
    ...
    { //need to test email activation
        ...
        if (!$email_activation || $active == '1') {
            ...
            if (!$this->config->item('use_ci_email', 'ion_auth')) {
                ...
                //$this->sma->send_email($email, $subject, $message); jm 
	...
	//register top smith
    public function register_inactive($username, $password, $email, $additional_data = array(), $active = FALSE, $notify = FALSE)
    { //need to test email activation
        $this->auth_model->trigger_events('pre_account_creation');
    ...
    public function getProfile($seller_id = null)
    {
        $ProfilebyPhone = $this->auth_model->getProfilebyPhone($seller_id);
