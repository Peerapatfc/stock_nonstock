...
class Main extends MY_Shop_Controller
{
    ...
    function index($act = NULL) {
        ...
        $data['id'] = $act;
        $this->load->view('default/main_wrapper',$data);
    }
	
    function profile($seller_id = NULL) {
		$user = $this->ion_auth->getProfile($seller_id);
		$getTeam = $this->ion_auth->getTeam($user[0]->parent_id);
		// $getPrivilege = $this->ion_auth->getPrivilege($user[0]->privilege_id);
		// $this->data['getPrivilege'] = $getPrivilege;
		
		$this->data['getTeam'] = $getTeam;
        $this->data['user'] = $user[0];
        $this->data['page_title'] = lang('profile');
        #$this->data['page_desc'] = $this->shop_settings->description;
        $this->page_construct('user/profile', $this->data);
		$this->load->view($this->theme . 'default/frontend/profile', $this->data);
    }
