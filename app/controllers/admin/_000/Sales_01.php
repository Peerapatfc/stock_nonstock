...
class Sales extends MY_Controller
{
    ...
    public function getSales($warehouse_id = null)
    {
        ...
        if ($warehouse_id) {
            $this->datatables
                ->select("{$this->db->dbprefix('sales')}.id as id, DATE_FORMAT({$this->db->dbprefix('sales')}.date, '%Y-%m-%d %T') as date, reference_no, biller, {$this->db->dbprefix('sales')}.customer, sale_status, grand_total, paid, (grand_total-paid) as balance, payment_status, {$this->db->dbprefix('sales')}.attachment, is_delete")
                ->from('sales')
		...
		#$this->datatables->where('return_id !=', 'NULL');
    ...
    function setdelete($id = NULL)
    {
        ...
        if ($this->form_validation->run() == true && $this->sales_model->Setdelete($id, $data)) {
            $this->session->set_flashdata('message', lang("sales_setdelete"));
            admin_redirect("sales");
        } else {
    ...
    public function add($quote_id = null)
    {
        ...
        if ($this->form_validation->run() == true) {
			...
			lang('Name : ') . $customer . "\n\r<br/>" .
			lang('bank from : ') . $value_bank_from . "\n\r<br/>" .
			lang('bank to : ') . $value_bank_to . "\n\r<br/>" .
			lang('Transfer Date : ') .  $date_cf_payment. "\n\r<br/>" .
			lang('Amount Transferred : ') . $total_cf_payment . "\n\r<br/>";	
            ...
            $data = array('date' => $date,
                ...
                'bank_from' => $bank_from,
				'bank_to' => $bank_to,
				'date_cf_payment' => $date_cf_payment,
    ...
    public function delivery_actions()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (!empty($_POST['val'])) {
                ...
                if ($this->input->post('form_action') == 'print_packing') {
                    foreach ($_POST['val'] as $id) {
						redirect('admin/sales/pdf_delivery/'.$id, '<i class="fa fa-file-pdf-o"></i> ' . lang('download_pdf'));
                    }
                    $this->session->set_flashdata('message', lang("print_packing"));
                    redirect($_SERVER["HTTP_REFERER"]);
                }
				/* action packing */
                if ($this->input->post('form_action') == 'packing') {
                    foreach ($_POST['val'] as $id) {
						/*$dlDetails = array(
							'status' => 'packing',
							'updated_at' => date("Y-m-d h:i:sa"), #2017-07-31 17:12:00
						...
						if ($this->sales_model->updateDelivery($id, $dlDetails)) {
							$this->session->set_flashdata('message', lang("delivery_updated"));
						}*/
                    }
                    $this->session->set_flashdata('message', lang("packing"));
                    redirect($_SERVER["HTTP_REFERER"]);
                }
