...
$this->load->admin_model('sequence_model');
    ...
    function getUsers()
    {
        ...
        $this->datatables
            ->select($this->db->dbprefix('users').".id as id, users.seller_id, first_name, last_name, email, company, " . $this->db->dbprefix('groups') . ".name, active")
            ->from("users")
...
function getAutoSellerId(){echo 0;
	...
	function register()
	{
		...
		if ($this->form_validation->run() == true) {
            ...
            $this->session->set_flashdata('message', "สมัครสมาชิกเรียบร้อยแล้ว กรุณารอทางผู้ดูแลระบบตรวจสอบข้อมูล เพื่อเปิดใช้งาน");
            admin_redirect("login");
