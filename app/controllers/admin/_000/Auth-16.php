...
$this->load->admin_model('sequence_model');
    ...
    function getUsers()
    {
		...
		if($this->Owner){
			...
			$this->datatables
            ->select($this->db->dbprefix('users').".id as id, users.seller_id, first_name, last_name, email, company, " . $this->db->dbprefix('groups') . ".name, active , users.default_dealer")
            ->from("users")
		    ...
		    //->add_column("Default Dealer", "<div class=\"text-center\"><a href='" . admin_url('auth/default_dealer/$1') . "' class='tip' title='" . lang("default_dealer") . "'><i class='fa fa-star-o' aria-hidden='true'></i></a></div>", "id")
...
function getAutoSellerId(){echo 0;
    ...
    function edit_user($id = NULL)
    {
		...
		// if(!($this->session->userdata('user_id')==$setting->default_admin_user)):
			// if($user_data->parent_id != "72"):
				// if (!$this->loggedIn || !$this->Owner && $id != $this->session->userdata('user_id')) {
					// $this->session->set_flashdata('warning', lang("access_denied"));
					// redirect($_SERVER["HTTP_REFERER"]);
		...
		if ($this->form_validation->run() == true) {
			...
			if($biller_id > 0){
				...
				$additional_data = array(
					...
					'agent' => $this->input->post('agent'),
					'vendor_inventory' => 2,
					'parent_id' => $this->input->post('id_advisor'),
					'card_no' => $card_no,
				);


				//if ($this->ion_auth->register_inactive($username, $password, $email, $additional_data, $active, $notify)) {
				...
				if($this->ion_auth->register($username, $password, $email, $additional_data, $active, $notify)){
					...
					$this->session->set_flashdata('message', "register_success_full");
					admin_redirect("login");
					//$this->load->view('default/admin/views/auth/success_full_regis', $data_user, false);
				}else{
					$this->session->set_flashdata('error', "register_error");
					redirect($_SERVER["HTTP_REFERER"]);
				}
			}
