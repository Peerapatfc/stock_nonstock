...
class Reports extends MY_Controller
{
    ...
    function getCommissionReport($pdf = NULL, $xls = NULL)
    {
        ...
        if ($pdf || $xls) {
            ...
            $this->load->library('datatables');
            $this->datatables
                ->select("DATE_FORMAT(date, '%Y-%m-%d %T') as date, reference_no, biller, customer, FSI.item_nane as iname, grand_total, commission_matrixrate.price, commission_matrixrate.price*grand_total/100  as balance, payment_status, {$this->db->dbprefix('sales')}.id as id", FALSE)
                ->from('sales')
                ->join($si, 'FSI.sale_id=sales.id', 'left')
                ->join('warehouses', 'warehouses.id=sales.warehouse_id', 'left')
				//custom
                ->join('users', 'users.id=sales.created_by', 'left')
				->join('commission_matrixrate', 'commission_matrixrate.type=users.type_commission', 'left')
				->where('sales.payment_status', 'paid')

				->where('commission_matrixrate.condition_from_value <= sales.grand_total')
				->where('commission_matrixrate.condition_to_value >= sales.grand_total');
                // ->group_by('sales.id');
