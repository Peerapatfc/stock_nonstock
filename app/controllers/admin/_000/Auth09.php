			...
			if ($this->form_validation->run() == true && $this->ion_auth->register($username, $password, $email, $additional_data, $active, $notify)) {
				...
				//if(){
					...
					if($this->object_settings_model->addWarehouse($warehouse_data)){
						$warehouse = $this->site->getWarehouseByCode($username."wh");
		...
		if ($this->form_validation->run() == true) {
			...
			if($biller_id > 0){
				...
				if ($this->ion_auth->register_inactive($username, $password, $email, $additional_data, $active, $notify)) {
					...
					$this->session->set_flashdata('message', "สมัครสมาชิกเรียบร้อยแล้ว กรุณารอผู้แนะนำเปิดการใช้งาน");
					admin_redirect("login");
					//$this->load->view('default/admin/views/auth/success_full_regis', $data_user, false);
				}else{
					$this->session->set_flashdata('error', "สมัครสมาชิกไม่สำเร็จ กรุณาแจ้งผู้ดูแลระบบ");
					redirect($_SERVER["HTTP_REFERER"]);
				}
			}
