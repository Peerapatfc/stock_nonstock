...
class Sales extends MY_Controller
{
    ...
    public function getSales($warehouse_id = null)
    {
        ...
        if ($warehouse_id) {
            ...
            $this->datatables
                ->select("{$this->db->dbprefix('sales')}.id as id, DATE_FORMAT({$this->db->dbprefix('sales')}.date, '%Y-%m-%d %T') as date, reference_no, biller, {$this->db->dbprefix('sales')}.customer, sale_status, grand_total, paid, (grand_total-paid) as balance, payment_status,deliveries.tracking, {$this->db->dbprefix('sales')}.attachment, {$this->db->dbprefix('sales')}.is_delete")
                ->from('sales')->join('deliveries', 'deliveries.sale_id = sales.id', 'left');
        }
		//->join('deliveries', 'deliveries.sale_id = sales.id', 'left');
		...
		#$this->datatables->add_column("is_deliveries", $is_deliveries, "id");
