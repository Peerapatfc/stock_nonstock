...
class Auth extends MY_Controller
{
    ...
    function getAwardpoints()
    {
        ...
        $this->datatables
			...
			if ($this->Owner || $this->Admin) {
				...
				$this->datatables->where('award_points.user_id', $this->session->userdata('user_id'));
			}
    ...
    function exchangepoints($id = NULL)
    {
        ...
        $data = array(
            ...
            'points_current' => $point * (-1),
			'points_rule_id' => $id,
			'date_insertion' => $date,
			'spent_points' => $point,
        );

		if($this->auth_model->exchangepoints($data)){
    ...
    function approvepoints($id = NULL)
    {
        $data = array(
			'approve' 	=> "1",
			'id' 		=> $id ,
        );
		if($this->auth_model->updatepoints($data)){
