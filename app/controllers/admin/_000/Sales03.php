...
class Sales extends MY_Controller
{
    ...
    public function getSales($warehouse_id = null)
    {
		...
		#$is_deliveries = $this->sales_model->getisdeliveries('$1'); getsales
				// {$this->db->dbprefix('sales')}.cms_sync_web,
				// companies.line,
				// companies.facebook,
        ...
        if ($warehouse_id) {
            $this->datatables
                ->select("{$this->db->dbprefix('sales')}.id as id,  DATE_FORMAT({$this->db->dbprefix('sales')}.date, '%Y-%m-%d %T') as date,{$this->db->dbprefix('sales')}.cms_sync_web as cms_sync_web, reference_no, biller, {$this->db->dbprefix('sales')}.customer, sale_status, grand_total, paid, (grand_total-paid) as balance, payment_status,deliveries.tracking, {$this->db->dbprefix('sales')}.attachment, {$this->db->dbprefix('sales')}.is_deliveries,{$this->db->dbprefix('sales')}.is_delete,  companies.line, companies.facebook")
                ->from('sales')
    ...
    public function add($quote_id = null)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $data = array('date' => $date,
                ...
                //'bank_from' => $bank_from,
				'bank_to' => $bank_to, 
				'date_cf_payment' => $date_cf_payment." ".$time_cf_payment, //5555555555555 2017-11-08 12:15:04
