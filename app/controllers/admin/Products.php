...
class Products extends MY_Controller
{
    ...
    function getProducts($warehouse_id = NULL)
    {
        ...
        if (!$this->Owner && !$this->Admin) {
			$brand_targets = $salebrand['brand_targets'];
			$brand_targets = explode(",", $brand_targets);
			$brand_targets = implode(",", $brand_targets);
			$this->datatables->where("products.brand IN ($brand_targets)");
        }
    ...
    function add($id = NULL)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $data = array(
				...
				'award_points' => $this->input->post('point_reward'),
            );
			
		$datawhosales = array();
            if($this->input->post('wholesale_quantity')) {
                foreach($this->input->post('wholesale_quantity') as $key => $whosale_quantity) {
                    $datawhosales[] = array(
                        'quantity' => $whosale_quantity,
                        'price' => $this->input->post('wholesale_price')[$key],
                    );
                }
            }
        ...
        if ($this->form_validation->run() == true && $this->products_model->addProduct($data, $items, $warehouse_qty, $product_attributes, $photos , $datawhosales)) {
    ...
    function edit($id = NULL)
    {
		 ...
		 $datawhosales = array();
            if($this->input->post('wholesale_quantity')) {
                foreach($this->input->post('wholesale_quantity') as $key => $whosale_quantity) {
                    ...
                    if(isset($this->input->post('id_wholesale')[$key])) {
                        $whole['id'] = $this->input->post('id_wholesale')[$key];
                    }
                    $datawhosales[] = $whole;
                }
            }
		//$this->sma->print_arrays($datawhosales);
        ...
        if ($this->form_validation->run() == true && $this->products_model->updateProduct($id, $data, $items, $warehouse_qty, $product_attributes, $photos, $update_variants,$datawhosales)) {
		  ...
		  $this->data['product_wholesales'] = $this->products_model->getProductWholesales($id);
