   ...
   function index()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $data = array('site_name' => DEMO ? 'Stock Manager Advance' : $this->input->post('site_name'),
				...
				'web_sync_url' => $this->input->post('web_sync_url'),
                'sms_template' => strip_tags($this->input->post('sms_template')), // for sms template
				'default_shipping' => $this->input->post('default_shipping'),
				'wallet_use' => $this->input->post('wallet_use'),
				'award_use' => $this->input->post('award_use'),
				...
				'billing_addr' => $this->input->post('billing_addr'),
				'billing_phone' => $this->input->post('billing_phone'),
				'supplier' => $this->input->post('supplier'),
				'default_admin_user' => $this->input->post('default_admin_user'),
            );
        ...
        if ($this->form_validation->run() == true && $this->settings_model->updateSetting($data)) {
			...
			$_setting = $this->settings_model->getSettings();
            $this->data['error'] = validation_errors() ? validation_errors() : $this->session->flashdata('error');
            $this->data['billers'] = $this->site->getAllCompanies('biller');
			$this->data['suppliers'] = $this->site->getAllCompanies('supplier');
            $this->data['settings'] = $_setting;
		 ...
		 $this->data['users'] = $this->site->getUserAllSetting();
    ...
    {
        $this->data['error'] = validation_errors() ? validation_errors() : $this->session->flashdata('error');
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('system_settings'), 'page' => lang('system_settings')), array('link' => '#', 'page' => lang('commission')));
        $meta = array('page_title' => lang('commission'), 'bc' => $bc);
        $this->page_construct('settings/commission', $meta, $this->data);
    }
    ...
    function getCommission()
    {
        ...
        $this->load->library('datatables');
        $this->datatables
            ->select("{$this->db->dbprefix('commission_matrixrate')}.id as id, {$this->db->dbprefix('commission_matrixrate')}.condition_from_value, {$this->db->dbprefix('commission_matrixrate')}.condition_to_value,  {$this->db->dbprefix('commission_matrixrate')}.price, {$this->db->dbprefix('commission_matrixrate')}.type", FALSE)
            ->from("commission_matrixrate")
            #->add_column("Actions", "<div class=\"text-center\"><a href='" . admin_url('system_settings/edit_commission/$1') . "' data-toggle='modal' data-target='#myModal' class='tip' title='" . lang("edit_commission") . "'><i class=\"fa fa-edit\"></i></a> <a href='#' class='tip po' title='<b>" . lang("delete_commission") . "</b>' data-content=\"<p>" . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete' href='" . admin_url('system_settings/delete_commission/$1') . "'>" . lang('i_m_sure') . "</a> <button class='btn po-close'>" . lang('no') . "</button>\"  rel='popover'><i class=\"fa fa-trash-o\"></i></a></div>", "id");
			->add_column("Actions", "<div class=\"text-center\"><a href='" . admin_url('system_settings/edit_commission/$1') . "' data-toggle='modal' data-target='#myModal' class='tip' title='" . lang("edit_commission") . "'><i class=\"fa fa-edit\"></i></a> <a href='#' class='tip po' title='<b>" . $this->lang->line("delete_commission") . "</b>' data-content=\"<p>" . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete' href='" . admin_url('system_settings/delete_commission/$1') . "'>" . lang('i_m_sure') . "</a> <button class='btn po-close'>" . lang('no') . "</button>\"  rel='popover'><i class=\"fa fa-trash-o\"></i></a></div>", "id");
        echo $this->datatables->generate();
    }
    ...
    function add_commission()
    {
        ...
        } elseif ($this->input->post('condition_from_value')) {
            $this->session->set_flashdata('error', validation_errors());
            admin_redirect("system_settings/commission");
        }

        if ($this->form_validation->run() == true && $this->settings_model->commissionadded($data)) {
            $this->session->set_flashdata('message', lang("commission_added"));
            admin_redirect("system_settings/commission");
        } else {
            $this->data['error'] = validation_errors() ? validation_errors() : $this->session->flashdata('error');
            $this->data['base_units'] = $this->site->getAllBaseUnits();
            $this->data['modal_js'] = $this->site->modal_js();
            $this->load->view($this->theme . 'settings/add_commission', $this->data);
    ...
    function edit_commission($id = NULL)
    {
        ...
        if ($this->form_validation->run() == true && $this->settings_model->updateCommission($id, $data)) {
            $this->session->set_flashdata('message', lang("commission_updated"));
            admin_redirect("system_settings/commission");
        } else {
			$this->data['details'] = $this->settings_model->getCommissionByID($id);
            $this->data['id'] = $id;
            $this->data['modal_js'] = $this->site->modal_js();
            $this->data['error'] = validation_errors();
            $this->load->view($this->theme . 'settings/edit_commission', $this->data);
    ...
    function delete_commission($id = NULL)
    {
        if (!$this->Owner) {
            $this->session->set_flashdata('warning', lang('access_denied'));
            redirect($_SERVER["HTTP_REFERER"]);
        ...
        if ($this->settings_model->deleteCommission($id)) {
            $this->sma->send_json(array('error' => 0, 'msg' => lang("commission_deleted")));
        }
    }
	
    function commission_actions()
    {
        $this->form_validation->set_rules('form_action', lang("form_action"), 'required');
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (!empty($_POST['val'])) {
                if ($this->input->post('form_action') == 'delete') {
                    foreach ($_POST['val'] as $id) {
                        $this->settings_model->deleteCommission($id);
                    }
    ...
    function getShipping()
    {
        ...
        $this->datatables
            ...
            ->add_column("Actions", "<div class=\"text-center\"><a href='" . admin_url('system_settings/edit_shipping/$1') . "' data-toggle='modal' data-target='#myModal' class='tip' title='" . lang("edit_shipping") . "'><i class=\"fa fa-edit\"></i></a> <a href='#' class='tip po' title='<b>" . lang("delete_shipping") . "</b>' data-content=\"<p>" . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete' href='" . admin_url('system_settings/delete_shipping/$1') . "'>" . lang('i_m_sure') . "</a> <button class='btn po-close'>" . lang('no') . "</button>\"  rel='popover'><i class=\"fa fa-trash-o\"></i></a></div>", "id");
    ...
    function add_shipping()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $this->session->set_flashdata('error', validation_errors());
            admin_redirect("system_settings/shipping_matrixrate");
        }
        if ($this->form_validation->run() == true && $this->settings_model->addShipping($data)) {
    ...
    function edit_shipping($id = NULL)
    {
        ...
        if ($this->form_validation->run() == true && $this->settings_model->updateshipping($id, $data)) {
			...
			$this->data['shipping'] = $this->settings_model->getShippingByID($id);
            $this->data['id'] = $id;
            $this->data['modal_js'] = $this->site->modal_js();
            $this->data['error'] = validation_errors();
    ...
    function updateshipping()
    {
		...
		$data = $this->settings_model->updateshipping($id , $data);
