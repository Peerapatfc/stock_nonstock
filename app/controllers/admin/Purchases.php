...
class Purchases extends MY_Controller
{
    ...
    public function index($warehouse_id = null)
    {
      //  $this->sma->checkPermissions();
        ...
        if ($this->Owner || $this->Admin || !$this->session->userdata('warehouse_id')) {
			...
			$parent_id = $this->site->getUserByParent($this->session->userdata('user_id'));
			/* Check Child user for filter by Warehouse */
			if(count($parent_id)>"0"){
				$warehouse_data = array();
				$warehouse_data[]  = $this->site->getWarehouseByID($this->session->userdata('warehouse_id'));
				foreach($parent_id as $pid){
					if($pid->warehouse_id){
						$warehouse_data[] =  $this->site->getWarehouseByID($pid->warehouse_id);
					}
				}
		...
		$group_id = $this->site->getUserGroup($user_id);
		$this->data['group_id'] = $group_id;
		$this->data['purchases_setting'] = $this->purchases_model->getPurchasesSetting();
    ...
    public function getPurchases($warehouse_id = null)
    {
     //   $this->sma->checkPermissions('index');
        ...
        if ((!$this->Owner || !$this->Admin) && !$warehouse_id && !$this->session->userdata('view_right')) {
	   ...
	   $add_delivery_link = anchor('admin/purchases/add_delivery/$1', '<i class="fa fa-truck"></i> ' . lang('add_delivery'), 'data-toggle="modal" data-target="#myModal"');
        ...
        <ul class="dropdown-menu pull-right" role="menu">
            <li>' . $detail_link . '</li>
            <li>' . $payments_link . '</li>
            <li class="payment-link">' . $add_payment_link . '</li>
		  <li class="delivery-link" id="deli-$1">' . $add_delivery_link . '</li>
            <li>' . $edit_link . '</li>
            <li>' . $pdf_link . '</li>
            <li>' . $email_link . '</li>
            <li>' . $print_barcode . '</li>
     ...
     /*   if ($warehouse_id) {
			...
			if(count($parent_id)>"0"){
				foreach($parent_id as $pid){
					$allwarehouse .= '"'.$pid->warehouse_id.'",';
				}
				//$allwarehouse .= $warehouse_id;
				
				$this->datatables
					->select("id,  reference_no, supplier, create_user, status, grand_total, paid, (grand_total-paid) as balance, payment_status,  DATE_FORMAT(date, '%Y-%m-%d %T') as date, attachment")
					->from('purchases')
					->where('warehouse_id IN ('.$allwarehouse.')');
			}else{
		...
		$parent_id = $this->site->getUserByParent($this->session->userdata('user_id'));
		$allwarehousedata = array();
		if(count($parent_id)>"0"){
			foreach($parent_id as $pid){
				$allwarehousedata[] = $pid->warehouse_id;
				$allwarehouse .= '"'.$pid->warehouse_id.'",';
			}
			$allwarehouse .= '"'.$this->session->userdata('warehouse_id').'"';
			$allwarehousedata[] = $this->session->userdata('warehouse_id');
		}
		if($warehouse_id) {
			if(!$this->Owner || !$this->Admin){
				if(in_array($warehouse_id,$allwarehousedata)){
					$this->datatables
					->select("id,  reference_no, supplier, create_user, status, grand_total, paid, (grand_total-paid) as balance, payment_status,  DATE_FORMAT(date, '%Y-%m-%d %T') as date, attachment,supplier_id")
					->from('purchases')
					->where('warehouse_id', $warehouse_id);
				}else{
					$this->datatables
    ...
    {
		$attachmentByID = $this->purchases_model->getattachmentByID($id);
		if ($this->Owner || $this->Admin) {
			if ($attachmentByID["status"] == 'pending') {
				$this->add_payment($id);
				$this->data['show'] = 1;
			}else{
				$this->data['show'] = 0;
		...
		$this->data['id'] = $id;
        $this->data['pic'] = $attachmentByID;
        $this->data['modal_js'] = $this->site->modal_js();
        $this->data['error'] = validation_errors();
        $this->load->view($this->theme . 'purchases/popup', $this->data);
	}
    ...
    public function modal_view($purchase_id = null)
    {
       ...
       /* if (!$this->session->userdata('view_right')) {
		...
		$this->data['group_id'] = $group_id;
        $this->data['user_to'] = $this->site->getUser($inv->created_by);
	    $user_for_biller = $this->site->getUser($inv->created_by);
	    $this->data['user_to_address'] = $this->companies_model->getCompanyByEmail(($this->data['user_to']->email));
		
		//$this->data['user_to_username'] = $this->site->getCompanyByID(($user_for_biller->biller_id)+1);
	   
		$supplier = $this->site->getCompanyByID(($inv->supplier_id));
		$this->data['user_from_user'] = $this->site->getUserFromBiller($supplier->id);
		//echo $this->sma->print_arrays($this->data['user_from_user']);
		$this->data['user_from_company'] = $this->companies_model->getCompanyByID($supplier->id);
		
			
			//$this->data['user_from']
	 //   $this->data['user_from_address'] = $this->site->getCompanyByID(($inv->supplier_id));
	//    $this->data['user_from_username'] = $this->site->getUserByEmail($this->data['user_from_address']->email);
		
		// $this->data['user_from'] = $this->site->getUser($inv->created_by);
		// $this->data['user_to'] = $this->site->getUser($inv->created_by);
		...
		$this->data['additional_document']  =   $this->purchases_model->get_additional_document($purchase_id);
    ...
    public function purchases_setting()
    {
		$odprefix = $this->input->post('odprefix');
		$oddigits = $this->input->post('oddigits');
		$odcurrent = $this->input->post('odcurrent');
		
		$poprefix = $this->input->post('poprefix');
		$podigits = $this->input->post('podigits');
		$pocurrent = $this->input->post('pocurrent');
		
		$spprefix = $this->input->post('spprefix');
		$spdigits = $this->input->post('spdigits');
		$spcurrent = $this->input->post('spcurrent');
		...
		if ($this->purchases_model->updatePurchasesSetting($data)) {
            $this->session->set_flashdata('message', lang("purchases_setting_update_success"));
            redirect($_SERVER["HTTP_REFERER"]);
        } else {
            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
        }
...
//generate pdf for becurve
    ...
    public function pdf($purchase_id = null, $view = null, $save_bufffer = null)
    {
		...
		$purchases_setting = $this->purchases_model->getPurchasesSetting();
		$PurchasesReceipt = $this->purchases_model->getPurchasesReceipt($id, $purchases_setting->poprefix);
        // if (!$this->session->userdata('view_right')) {
            // $this->sma->view_rights($inv->created_by);
        // }
		$created_by = $this->site->getUser($inv->created_by);
		$Bille = $this->purchases_model->getBillerbyId($created_by->biller_id);
		if(isset($PurchasesReceipt->invoice_no)){
			$invoice_no = $PurchasesReceipt->invoice_no;
		}else{
			$sequence_no = sprintf("%0".$purchases_setting->podigits."d", $purchases_setting->pocurrent);
			$invoice_no = $purchases_setting->poprefix.''.date("Ym", strtotime($inv->date))."-".$sequence_no;
			
			if ($this->purchases_model->chkPurchases_Receipt($invoice_no)) {
				$this->session->set_flashdata('error', sprintf(lang("เลขที่ใบสั่งซื้อ (PO) %s ได้ถูกออกไปแล้ว รบกวนเปลี่ยนค่าใน เลขที่ใบเสร็จรับเงินปัจจุบัน"), $invoice_no));
				redirect($_SERVER["HTTP_REFERER"]);
			}

			$current_no = [
				'pocurrent' => $purchases_setting->pocurrent + 1,
			];
			...
			$sales_receipt = [
				...
				'type'			=> $purchases_setting->poprefix,
			];
			$this->purchases_model->addPurchasesReceipt($sales_receipt);
			$this->purchases_model->updatePurchasesSetting($current_no);
		}
		...
		$_customer 		= $this->site->getWarehouseByID($inv->warehouse_id);
        ...
        #foreach($query->result() as $key=>$row){
            ...
            $customer = [
				...
				'customerTax' => $Bille->vat_no,
				'customerCode' => $created_by->seller_id,
            ];

			$i = 0;
			$sale_items = $this->purchases_model->getAllPurchaseItems($purchase_id);
            foreach($sale_items as  $sale_item){	
                $order_data[$i++] = [
					'orderCode' 	=> $sale_item->product_code,
					'description' 	=> $sale_item->product_name,
					'quantity' 		=> number_format($sale_item->unit_quantity),
					'price' 		=> number_format($sale_item->unit_cost,2),
					'total' 		=> number_format($sale_item->subtotal,2),
				];
            }
			
			$vat = $inv->total*7/107;
			$grand_total = $inv->total-$vat;
			if ($Settings->tax2) {
				$summary = [
					//'cod'		=>	"",
					...
					'net_total'	=>	 number_format($inv->total, 2),
		...
		$html = $this->load->view($this->theme.'mpdf/po', $data, TRUE);
		$mpdf = new mPDF('utf-8', 'A4', '0', 'thsarabun');
		$mpdf->SetTitle('PO pdf');
        $mpdf->WriteHTML($html);
        $mpdf->Output();
    }
    ...
    public function pdf_od($purchase_id = null)
    {
        if ($this->input->get('id')) {
            $purchase_id = $this->input->get('id');
        }
		...
		$bank = $this->sales_model->getBanktranfer($bank_to);
		$purchases_setting = $this->purchases_model->getPurchasesSetting();
		$PurchasesReceipt = $this->purchases_model->getPurchasesReceipt($id, $purchases_setting->odprefix);
		$created_by = $this->site->getUser($inv->created_by);
		...
		if(isset($PurchasesReceipt->invoice_no)){
			...
			$sequence_no = sprintf("%0".$purchases_setting->oddigits."d", $purchases_setting->odcurrent);
			$invoice_no = $purchases_setting->odprefix.''.date("Ym", strtotime($inv->date))."-".$sequence_no;
			
			if ($this->purchases_model->chkPurchases_Receipt($invoice_no)) {
				$this->session->set_flashdata('error', sprintf(lang("เลขที่ใบเสร็จรับเงิน (Receipt) %s ได้ถูกออกไปแล้ว รบกวนเปลี่ยนค่าใน เลขที่ใบเสร็จรับเงินปัจจุบัน"), $invoice_no));
				redirect($_SERVER["HTTP_REFERER"]);
			}

			$current_no = [
				'odcurrent' => $purchases_setting->odcurrent + 1,
			];
			...
			$sales_receipt = [
				...
				'sale_id'		=> $inv->id,
				'invoice_no'	=> $invoice_no,
				'type'			=> $purchases_setting->odprefix,
			];
			$this->purchases_model->addPurchasesReceipt($sales_receipt);
			$this->purchases_model->updatePurchasesSetting($current_no);
		}
        ...
        #foreach($query->result() as $key=>$row){
            ...
            $customer = [
                'customerName' => $created_by->first_name." ".$created_by->last_name,
                'customerAddress' => $Bille->address,
                'telephone' => $created_by->phone,
				'customerTax' => $Bille->vat_no,
				'customerCode' => $created_by->seller_id,
            ];
			...
			$vat = $inv->total*7/107;
			$grand_total = $inv->grand_total - $vat;
				if ($Settings->tax2) {
				$summary = [
					//'cod'		=>	"",
					'shipping'	=>	 number_format($inv->shipping, 2),
    ...
    public function pdf_dv($purchase_id = null)
    {
        if ($this->input->get('id')) {
            $purchase_id = $this->input->get('id');
        }
		...
		$bank = $this->sales_model->getBanktranfer($bank_to);
		$purchases_setting = $this->purchases_model->getPurchasesSetting();
		$PurchasesReceipt = $this->purchases_model->getPurchasesReceipt($id, $purchases_setting->spprefix);
		$created_by = $this->site->getUser($inv->created_by);
		...
		if(isset($PurchasesReceipt->invoice_no)){
			...
			$sequence_no = sprintf("%0".$purchases_setting->spdigits."d", $purchases_setting->spcurrent);
			$invoice_no = $purchases_setting->spprefix.''.date("Ym", strtotime($inv->date))."-".$sequence_no;
			
			if ($this->purchases_model->chkPurchases_Receipt($invoice_no)) {
				$this->session->set_flashdata('error', sprintf(lang("เลขที่ ใบส่งของ (delivery order) %s ได้ถูกออกไปแล้ว รบกวนเปลี่ยนค่าใน เลขที่ใบเสร็จรับเงินปัจจุบัน"), $invoice_no));
				redirect($_SERVER["HTTP_REFERER"]);
			}

			$current_no = [
				'spcurrent' => $purchases_setting->spcurrent + 1,
			];
			...
			$sales_receipt = [
				...
				'sale_id'		=> $inv->id,
				'invoice_no'	=> $invoice_no,
				'type'			=> $purchases_setting->spprefix,
			];
			$this->purchases_model->addPurchasesReceipt($sales_receipt);
			$this->purchases_model->updatePurchasesSetting($current_no);
		}
		...
		$setting 		= $this->object_settings_model->getSettings();
		$order_no 		= $inv->reference_no;
		$_delivery		= $this->purchases_model->getDeliveryByPurchaseRef($inv->reference_no);
        ...
        #foreach($query->result() as $key=>$row){
            ...
            $header['setting'] = array(
                'codeToBarCode' => "<img src='" . admin_url('products/gen_barcode/' . $inv->reference_no) . "' alt='" . $inv->reference_no . "' class='pull-left' />",
				'orderdate' 	=> date("d/m/Y", strtotime($_delivery->date)),
				'orderno' 		=> $order_no,
				'invoiceno'		=> $invoice_no,
            );
			
			#print_r($_delivery); exit(0);
			$header['deliverycode'] = array(
				'purchaseorder'	=> $order_no,
				'deliveryorder'	=> $_delivery->do_reference_no,
				'saleman'		=> $inv->supplier,
			
			);
			
			$tracking = isset($_delivery->tracking) ? $_delivery->tracking : "-";
			$header['paymentchannel'] = array(
				'order'		=> $bank->bank,
				'payment'	=> "โอนเงินผ่านธนาคาร",
				'shipping'	=> $inv->delivery_type." (".$tracking.")",
		...
		$html = $this->load->view($this->theme.'mpdf/deliveryorder', $data, TRUE);
		$mpdf = new mPDF('utf-8', 'A4', '0', 'thsarabun');
		$mpdf->SetTitle('delivery order pdf');
        $mpdf->WriteHTML($html);
        $mpdf->Output();
    }
	...
	public function add_delivery($id = null)
    {
		...
		//echo $this->sma->print_arrays($id);
		$purchase = $this->purchases_model->getPurchaseByID($id);
		$user = $this->site->getUserByWarehouse($purchase->warehouse_id);
		$companies  = $this->companies_model->getCompanyByEmail($user->email);
		//echo $this->sma->print_arrays($purchase);
        ...
        if ($delivery = $this->purchases_model->getDeliveryByPurchaseRef($purchase->reference_no)) {
           ...
           $this->form_validation->set_rules('address', lang("address"), 'required');
               ...
               if ($this->Owner || $this->Admin) {
                ...
                $dlDetails = array(
                   ...
                   'sale_id' => $purchase->id,
                    ...
                    'sale_reference_no' => $purchase->reference_no,
				...
				'shipping_type' => $purchase->delivery_type,
				'updated_at' => date("Y-m-d h:i:sa"),
                    );
				//	echo $this->sma->print_arrays($dlDetails);
            ...
            if ($this->form_validation->run() == true && $this->sales_model->addDelivery($dlDetails)) {
				$this->purchases_model->isship($id,$is_ship);
                $this->session->set_flashdata('message', lang("delivery_added"));
                admin_redirect("sales/deliveries");
                #redirect($_SERVER["HTTP_REFERER"]);
            } else {
                $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
                $this->data['customer'] = ($user->first_name)." ".($user->last_name);
                $this->data['address'] = $companies->address;
                $this->data['inv'] = $purchase;
                $this->data['do_reference_no'] = ''; 
				
				//$this->site->getReference('do');
                $this->data['modal_js'] = $this->site->modal_js();
                $this->load->view($this->theme . 'purchases/add_delivery', $this->data);
		}
		//echo $this->sma->print_arrays($dlDetails);
	...
	public function edit_delivery($id = null)
    {
      //  $this->sma->checkPermissions();
        ...
        if ($this->form_validation->run() == true && $this->sales_model->updateDelivery($id, $dlDetails)) {
            ...
            $this->data['modal_js'] = $this->site->modal_js();
            $this->load->view($this->theme . 'purchases/edit_delivery', $this->data);
        }
    }
    ...
    public function add($quote_id = null)
    {
		...
		$user_id = $this->session->userdata('user_id');
		$group_id = $this->site->getUserGroup($user_id);
		$customer = $this->sales_model->customeridbyuser($user_id);
		
		$parent_id =  $this->site->getUser($user_id);
		$parent_user =  $this->site->getUser($parent_id->parent_id);
		
		$bank = $this->sales_model->getbanktransfer($parent_user->id);
		
		// if($user_id == 196){
			// $this->sma->print_arrays($parent_user);
		// }
        ...
        if ($this->form_validation->run() == true) {
			...
			$create_user = $customer[0]->name;
			$addr = $customer[0]->address;
		  ...
		  $product_point_sum = 0;
		   ...
		   $privilege_item = 0;
            for ($r = 0; $r < $i; $r++) {
				...
				$product_sum = $this->purchases_model->getProductByCode($item_code);
				//$this->sma->print_arrays($product_sum);
				$product_point_sum += $product_sum->award_points * $item_quantity;
                    $total += $this->sma->formatDecimal(($item_net_cost * $item_unit_quantity), 4);
					
				$privilege_item	+= $item_quantity;
                }
			...
			lang('ทำรายการโดย : ') . $customer[0]->name." - ".$customer[0]->phone . "\n\r<br/>" .
            ...
            $data = array('reference_no' => $reference,
               ...
               // 'supplier_id' => $supplier_id,
                'supplier_id' => $parent_user->biller_id,
               // 'supplier' => $supplier,
                'supplier' => $parent_user->first_name ." ". $parent_user->last_name,
                'warehouse_id' => $warehouse_id,
                'note' => $note,
                'total' => $total,
	...
	//		$this->sma->print_arrays($privilege_item);
			//$this->sma->print_arrays($privilege_item);
		/*	$privilege_id = $this->purchases_model->getPrivilege($privilege_item);
			if($privilege_id->id > $parent_id->privilege_id){
				$this->auth_model->updatePrivilege($user_id,$privilege_id->id);
			}*/
			
			//if($group_id->name=="wisdom"):
				if ($setting->award_use){
					$points_current = floor(($grand_total / $setting->each_sale) * $setting->sa_point);
					if($setting->award_use="2"){
						$points_current = $product_point_sum;
					}
					$data_purchase = array(
					...
					$this->purchases_model->addPoint($data_purchase);
				}
			//endif;
			admin_redirect('purchases');
        } else {
    ...
    public function edit($id = null)
    {
     ...
     /*   if (!$this->session->userdata('edit_right')) {
			if($setting->default_admin_user =  $this->session->set_userdata('user_id')){
				$this->sma->view_rights($inv->created_by);
			}
        }*/
        ...
        if ($this->form_validation->run() == true) {
		 ...
		 $bank = $this->sales_model->banktranfer();
            $shipping = $this->input->post('shipping_price');
            $supplier_details = $this->site->getCompanyByID($supplier_id);
            $supplier = $supplier_details->company != '-'  ? $supplier_details->company : $supplier_details->name;
            $note = $this->sma->clear_tags($this->input->post('note'));
            $payment_term = $this->input->post('payment_term');
        ...
        if ($this->form_validation->run() == true && $this->purchases_model->updatePurchase($id, $data, $products)) {
            ...
            foreach ($inv_items as $item) {
		      ...
		      $user = $this->site->getUser($user->id);
			 $user_parent = $this->site->getUser($user->parent_id);
			 $user_parent_id = $this->purchases_model->checkParentWarehouse($user->parent_id);
			 $product_parent_stock = $this->products_model->getProductParentStock($row->id,$user_parent->warehouse_id);
			 
                $pr[$ri] = array('id' => $c, 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")", 
                    'row' => $row, 'combo_items' => $combo_items, 'tax_rate' => $tax_rate, 'units' => $units, 'options' => $options, 'product_wholesales' => $product_wholesales , 'product_parent_qty' => $item->product_id);
                $c++;
            }

            $this->data['inv_items'] = json_encode($pr);
            $this->data['id'] = $id;
		 $this->data['bank'] =  $this->sales_model->banktranfer();
		 $this->data['shippingtitle'] = $this->sales_model->shippingtitle($this->Settings->default_shipping);
            $this->data['suppliers'] = $this->site->getAllCompanies('supplier');
        ...
        if (!empty($_POST['val'])) {
            /* action packing */
                ...
                foreach ($_POST['val'] as $id) {
                    ...
                    $this->purchases_model->updatePrint($id, "1", date("Y-m-d h:i:sa"));
                    #$this->session->set_flashdata('message', lang("print label for packing success"));
                }
                $sql = "
					SELECT 
						(sma_purchases.`id`) AS id,
                        (sma_warehouses.name) AS customer_name, 
                        (sma_warehouses.address) AS customer_address, 
                        (sma_warehouses.phone) AS customer_phone, 
                        (sma_purchases.reference_no) AS order_no,
                        (sma_purchases.`warehouse_id`) AS warehouse_id    
						FROM sma_warehouses 
						INNER JOIN sma_purchases ON sma_warehouses.id = sma_purchases.`warehouse_id`
                    
						WHERE sma_purchases.id IN ? 
                ";
                ...
                foreach($query->result() as $key => $row){
                    ...
                    // query sale items
					$purchase_items = $this->db->get_where('purchase_items', array('purchase_id' => $row->id));
                    foreach($purchase_items->result() as $purchase_items){
                        $header[$key]['product'][] = ['sku'=>$purchase_items->product_code, 'quantity'=>intval($purchase_items->quantity)];
                    }
			...
			if ($this->input->post('form_action') == 'owner_receiver') {
            ...
            $this->session->set_flashdata('error', lang("no_selected"));
            redirect($_SERVER["HTTP_REFERER"]);
		}
    }
    ...
    public function suggestions()
    {
        ...
        if ($rows) {
            ...
            foreach ($rows as $row) {
                ...
                $units = $this->site->getUnitsByBUID($row->base_unit);
                $tax_rate = $this->site->getTaxRateByID($row->tax_rate);
			$product_wholesales = $this->products_model->getProductWholesales($row->id);
			$user = $this->site->getUser($user->id);
			$user_parent = $this->site->getUser($user->parent_id);
			 $user_parent_id = $this->purchases_model->checkParentWarehouse($user->parent_id);
			 $product_parent_stock = $this->products_model->getProductParentStock($row->id,$user_parent->warehouse_id);
                $pr[] = array('id' => ($c + $r), 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")", 
                    'row' => $row, 'tax_rate' => $tax_rate, 'units' => $units, 'options' => $options,  'product_wholesales' => $product_wholesales , 'product_parent_qty' => $product_parent_stock );
                $r++;
    ...
    public function add_payment($id = null)
    {
        ...
        if ($this->form_validation->run() == true) {
			...
			$purchases = $this->purchases_model->getPurchaseByID($this->input->post('purchase_id'));
			$point_data = array(
				'reference_no' => $purchases->reference_no
			);
			
			//if ($setting->award_use) {
				$this->purchases_model->update_status_points($point_data);
		//	}
		
		
		

          //  $this->sma->print_arrays($point_data);
        ...
        if ($this->form_validation->run() == true && $this->purchases_model->addPayment($payment) && $this->purchases_model->updateStatus($id, "received", $this->sma->clear_tags($this->input->post('note')))) {
        //if ($this->form_validation->run() == true) {
		$users_create = $this->site->getUser($purchase->created_by);
		if($users_create->parent_id){
			$parent_user = $this->site->getUser($users_create->parent_id);
			$this->purchases_model->adjustParentStock($this->purchases_model->getAllPurchaseItems($id),$parent_user->warehouse_id);
		}
		
		/* Update privilege level by number of item ordered */
			$purchase_item = $this->purchases_model->getAllPurchaseItems($this->input->post('purchase_id'));
			$item_ordered = 0;
			foreach($purchase_item as $pur_item){
				$item_ordered += $pur_item->quantity_received;
			}
			//$this->sma->print_arrays($item_ordered);
			$privilege_id = $this->auth_model->getPrivilege($item_ordered);
			$user_privilege = $this->site->getUser($purchase->created_by);
			if($privilege_id->id > $user_privilege->privilege_id){
				$this->auth_model->updatePrivilege($purchase->created_by,$privilege_id->id);
			}
    ...
    public function update_status($id)
    {
	  ...
	  public function additional_document($purchase_id = null)
		{
		   ...
		   if ($_FILES['document']['size'] > 0) {
			...
			if ($this->purchases_model->additional_document($id, $data)) {
				$this->session->set_flashdata('message', lang('status_updated'));
				admin_redirect(isset($_SERVER["HTTP_REFERER"]) ? $_SERVER["HTTP_REFERER"] : 'purchases');
			}
			//$this->sma->print_arrays($_POST['purchase_id']);
