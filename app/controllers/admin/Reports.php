...
class Reports extends MY_Controller
{
    ...
    function daily_report()
    {
        ...
        $this->data['warehouses'] = $this->site->getAllWarehouses();
        $this->data['billers'] = $this->site->getAllCompanies('biller');
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('reports'), 'page' => lang('reports')), array('link' => '#', 'page' => lang('daily_report')));
        $meta = array('page_title' => lang('daily_report'), 'bc' => $bc);
        $this->page_construct('reports/daily_report', $meta, $this->data);
    }
	
	
    function getDailyReport($pdf = NULL, $xls = NULL)
    {
        $this->sma->checkPermissions('sales', TRUE);
        $product = $this->input->get('product') ? $this->input->get('product') : NULL;
        $user = $this->input->get('user') ? $this->input->get('user') : NULL;
        ...
        if ($pdf || $xls) {
            $this->db
				...
				biller, 
				grand_total,  
				banktransfer.bank as bank, 
				banktransfer.account_number as account_number, 
				{$this->db->dbprefix('sales')}.id as id",
				FALSE)
				
                ->from('sales')
                ->join('banktransfer', 'banktransfer.id=sales.bank_to', 'left')
                ->order_by('sales.date desc');
            ...
            if (!empty($data)) {
				...
				$this->excel->getProperties()->setCreator("ตัวแทนจำหน่าย");
				$this->excel->getProperties()->setLastModifiedBy("ตัวแทนจำหน่าย");
				$this->excel->getProperties()->setTitle("ยอดขายรายวัน");
				$this->excel->getProperties()->setSubject("ยอดขายรายวัน");
				$this->excel->getProperties()->setDescription("ยอดขายรายวัน Daily Report");
                ...
                $this->excel->getActiveSheet()->SetCellValue('E1', lang('bank'));
                $this->excel->getActiveSheet()->SetCellValue('F1', lang('account_number'));
                // $this->excel->getActiveSheet()->SetCellValue('G1', lang('paid'));
                // $this->excel->getActiveSheet()->SetCellValue('H1', lang('balance'));
                // $this->excel->getActiveSheet()->SetCellValue('I1', lang('payment_status'));
                ...
                foreach ($data as $data_row) {
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('E' . $row, $data_row->bank);
                    $this->excel->getActiveSheet()->SetCellValue('F' . $row, $data_row->account_number);
                    // $this->excel->getActiveSheet()->SetCellValue('G' . $row, $data_row->paid);
                    // $this->excel->getActiveSheet()->SetCellValue('H' . $row, ($data_row->grand_total - $data_row->paid));
                    // $this->excel->getActiveSheet()->SetCellValue('I' . $row, lang($data_row->payment_status));
                    $total += $data_row->grand_total;
				...
				$this->excel->getActiveSheet()->getStyle('A1:F1')->applyFromArray($styleHeader);
            ...
            $this->load->library('datatables');
            $this->datatables
                ->select("{$this->db->dbprefix('sales')}.date, reference_no, biller, grand_total,  banktransfer.bank, banktransfer.account_number, {$this->db->dbprefix('sales')}.id as id", FALSE)
                ->from('sales')
                ->join('banktransfer', 'banktransfer.id=sales.bank_to', 'left');
			//$this->db->where('sales.sale_status', 'completed');
    ...
    function commission()
    {
        ...
        $this->data['warehouses'] = $this->site->getAllWarehouses();
        $this->data['billers'] = $this->site->getAllCompanies('biller');
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('reports'), 'page' => lang('reports')), array('link' => '#', 'page' => lang('commission_report')));
        $meta = array('page_title' => lang('sales_report'), 'bc' => $bc);
        $this->page_construct('reports/commission', $meta, $this->data);
    }
	
    
    function getCommissionReport($pdf = NULL, $xls = NULL)
    {
        $this->sma->checkPermissions('sales', TRUE);
        $product = $this->input->get('product') ? $this->input->get('product') : NULL;
        $user = $this->input->get('user') ? $this->input->get('user') : NULL;
    ...
    function getUsers()
    {
        $this->load->library('datatables');
        $this->datatables
            ->select($this->db->dbprefix('users').".id as id, seller_id, first_name, last_name, email, company, ".$this->db->dbprefix('groups').".name, active")
            ->from("users")
    ...
    function orderProvince()
    {
        #$this->sma->checkPermissions('sales');
        $province = $this->input->post('province') ? $this->input->post('province') : NULL;
        $start_date = $this->input->post('start_date') ? $this->input->post('start_date') : NULL;
        $end_date = $this->input->post('end_date') ? $this->input->post('end_date') : NULL;
		if($province){
			$zip = array();
			$zip_code = $this->reports_model->getZipcodebyProvince($province);
		...
		$salesbyzipcode = $this->reports_model->getSalesByZipcode($zip, $start_date, $end_date);
		$rows = array();
		$dataLat = array();
		foreach($salesbyzipcode as $sales){
			$rows = $this->reports_model->getOrderProvinceByzipcode($sales->zipcode);
			$rows += array("grand_total" => number_format($sales->grand_total, 2));
			$rows += array("count_order" => $rows['name_in_thai'].": ".number_format($sales->count_order));
			$dataLat[] = $rows;
		}

        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
		$this->data['users'] = $this->reports_model->getStaff();
        $this->data['dataLat'] = $dataLat;
        $this->data['warehouses'] = $this->site->getAllWarehouses();
        $this->data['province'] = $this->reports_model->getAllProvince();
		
		
        #$jsonLat = json_encode($dataLat);
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('reports'), 'page' => lang('reports')), array('link' => '#', 'page' => lang('order_province')));
        $meta = array('page_title' => lang('orderprovince'), 'bc' => $bc);
        $this->page_construct('reports/orderprovince', $meta, $this->data);
    }
	
	
	
    function getOrderProvinceReport($pdf = NULL, $xls = NULL)
    {
        #$this->sma->checkPermissions('sales', TRUE);
        ...
        if ($pdf || $xls) {
            ...
            $this->load->library('datatables');
			$wp = "(SELECT {$this->db->dbprefix('addr_provinces')}.name_in_thai, {$this->db->dbprefix('addr_subdistricts')}.zip_code
					FROM {$this->db->dbprefix('addr_subdistricts')}
					LEFT JOIN {$this->db->dbprefix('addr_districts')} ON {$this->db->dbprefix('addr_districts')}.id = {$this->db->dbprefix('addr_subdistricts')}.district_id
					LEFT JOIN {$this->db->dbprefix('addr_provinces')} ON {$this->db->dbprefix('addr_provinces')}.id = {$this->db->dbprefix('addr_districts')}.province_id
					GROUP BY {$this->db->dbprefix('addr_subdistricts')}.zip_code) FWP";
            
			$this->datatables
                ->select("name_in_thai, count(id), sum(grand_total)", FALSE)
				->join($wp, 'FWP.zip_code = sales.zipcode', 'left')
                ->from('sales');

			//$this->db->where('sales.sale_status', 'completed');
			if($province){
				$this->datatables->where('name_in_thai', $province);
			}
			...
			$this->datatables->group_by('name_in_thai');
            echo $this->datatables->generate();
        }
    }
