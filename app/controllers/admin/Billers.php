...
class Billers extends MY_Controller
{
    ...
    function add()
    {
		...
		#max_limit
		$limitbiller = $this->sales_model->getLimitby('biller');
		$limitbiller_total = $limitbiller->total_group;
		$setting = $this->object_settings_model->getSettings();
		$limitbybiller = $setting->limitbybiller;
		
		if($limitbiller_total >= $limitbybiller):
			$max_limit = true;
		else:
			$max_limit = false;
		endif;
		...
		if ($this->form_validation->run() == true && $this->companies_model->addCompany($data)) {
			...
			$this->data['limitbybiller'] = $limitbybiller;
