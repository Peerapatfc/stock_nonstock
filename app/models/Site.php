...
class Site extends CI_Model
{
	...
	public function getUserFromBiller($id) {
 
        $q = $this->db->get_where('users', array('biller_id' => $id), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
    ...
    public function getUserBy($seller_id = NULL) {
	...
	public function getUserAll($id = NULL) {
       /* if (!$id) {
            $id = $this->session->userdata('user_id');
        }*/
        $this->db->select('id, first_name, last_name, avatar, seller_id, gender, img');
	...
	public function getUserAllSetting($id = NULL) {
       /* if (!$id) {
            $id = $this->session->userdata('user_id');
        }*/
		 $q = $this->db->get('users');
	...
	public function getUserByWarehouse($warehouse_id) {
        $q = $this->db->get_where('users', array('warehouse_id' => $warehouse_id), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
	...
	public function getUserByParent($id = NULL) {
        if (!$id) {
            $id = $this->session->userdata('user_id');
        }
        $q = $this->db->get_where('users', array('parent_id' => $id));
        if ($q->num_rows() > 0) {
           foreach (($q->result()) as $row) {
                $data[] = $row;
            }
    ...
    public function getReference($field) {
        ...
        if ($q->num_rows() > 0) {
            ...
            if ($this->Settings->reference_format == 1) {
                ...
                $ref_no = $prefix .= date('Y'). date('m') . "-" . sprintf("%03s", $ref->{$field});
    ...
    public function syncProductQty($product_id, $warehouse_id) {
        ...
        if ($this->db->update('products', array('quantity' => $balance_qty), array('id' => $product_id))) {
            if ($this->getWarehouseProducts($product_id, $warehouse_id)) {
                ...
                $this->db->update('purchase_items', array('quantity' => $wh_balance_qty), array('product_id' => $product_id, 'warehouse_id' => $warehouse_id));
            } else {
