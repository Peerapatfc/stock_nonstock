...
class Sales extends MY_Controller
{
    ...
    public function getSales($warehouse_id = null)
    {
        ...
        if ($warehouse_id) {
            $this->datatables
                ->select("{$this->db->dbprefix('sales')}.id as id,  ,{$this->db->dbprefix('sales')}.cms_sync_web as cms_sync_web, reference_no, biller, {$this->db->dbprefix('sales')}.customer, sale_status, grand_total, paid, (grand_total-paid) as balance, payment_status,deliveries.tracking, {$this->db->dbprefix('sales')}.attachment,DATE_FORMAT({$this->db->dbprefix('sales')}.date, '%Y-%m-%d %T') as date, {$this->db->dbprefix('sales')}.is_deliveries,{$this->db->dbprefix('sales')}.is_delete,  companies.line, companies.facebook")
                ->from('sales')
		...
		$view_right = 0;
		//$this->datatables->where('sales.created_by', 2);
		$view_right = $this->sales_model->viewright(10);
		//$this->datatables->where('sales.created_by', 2);
