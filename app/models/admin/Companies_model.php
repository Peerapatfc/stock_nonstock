...
class Companies_model extends CI_Model
{
    ...
    public function getCompaniesByEmail($email)
    {
	 ...
	 public function getCompanyByEmailAndAddress($email,$address)
    {
        $q = $this->db->get_where('companies', array('email' => $email , 'address' => $address), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
    ...
    public function getCustomerSuggestions($term, $limit = 10, $user_id)
    {
        $this->db->select("id, (CASE WHEN company = '-' THEN name ELSE CONCAT(name, ' (', phone, ')') END) as text", FALSE);
		...
		if (strlen(trim($user_id)) > 0) {
			$q = $this->db->get_where('companies', array('group_name' => 'customer', 'user_id' => $this->session->userdata('user_id')), $limit);
		}else{
