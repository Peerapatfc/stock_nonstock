...
class Sales_model extends CI_Model
{
    ...
    public function getProductNames($term, $warehouse_id, $brand_targets, $limit = 5)
    {
...
// SELECT `sma_companies`.id,`sma_users`.`email`,`sma_companies`.`email` FROM `sma_users`
// LEFT JOIN `sma_companies`
// ON `sma_users`.`email` = `sma_companies`.`email`

// WHERE `sma_companies`.`group_name` = 'user'

		$user_id = !is_null($id) ? $id : $this->session->userdata('user_id');
		
        $this->db->select('companies.*')
            ->join('users', 'users.email = companies.email', 'left');
        $q = $this->db->get_where('companies', array('group_name' => 'user', 'users.id' => $user_id));
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
    ...
    {
// SELECT * FROM `sma_shipping_matrixrate`
// WHERE `delivery_type` = 'ems' && `condition_name` = 'price' && (`condition_from_value` <= '500' && '500' <= `condition_to_value`)
    ...
    public function trackingDay($date)
    {
        ...
        $this->db->where("updated_at <=", "$date 23:59:59");
     ...
     public function trackingDate($date)
    {
        $this->db->select('deliveries.id as id, deliveries.date, deliveries.updated_at, deliveries.customer, deliveries.tracking', FALSE)
