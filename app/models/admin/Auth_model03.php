...
class Auth_model extends CI_Model
{
	...
	public function exchangepoints($data)
    {
    ...
    {
		#$this->db->update($this->tables['users'], $data, array('id' => $id));
		if ($this->db->update('award_points', $data, array('id' => $data['id']))) {
            return true;
        } else {
            return false;
        }
    ...
    {
        $this->db->select('sum(points_current) as points_current, sum(spent_points) as spent_points');
        $q = $this->db->get_where('award_points', array('award_points.user_id' => $this->session->userdata('user_id')));
        if ($q->num_rows() > 0) {
            // foreach (($q->result()) as $row) {
                // $data[] = $row;
            // }
	...
	public function update_wallet($data)
    {
		...
		if($data['wallet_type']=="deposit"){
		 ...
		 if ($this->db->insert("wallets", array('user_id' => $data['user_id'], 'amount' => $data['wallet_amount'], 'type' => $data['wallet_type'], 'approve' => "0",'upload_slip' => $data['upload_slip'] ))) {
            return true;
        } else {
            return false;
        }
