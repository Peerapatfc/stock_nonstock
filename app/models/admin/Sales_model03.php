...
class Sales_model extends CI_Model
{
    ...
    public function getProductNames($term, $warehouse_id, $brand_targets, $limit = 5)
    {
        ...
        $this->db->select('products.*, FWP.quantity as quantity, categories.id as category_id, categories.name as category_name', FALSE)
			...
			#$sql = 'SELECT * FROM `table`  WHERE `id` IN (' . implode(',', array_map('intval', $brand_targets)) . ')';
			#$this->db->where("{$this->db->dbprefix('products')}.brand IN (SELECT `users`.`brand_targets` FROM `users` WHERE id = 8)");
			#explode(",",$brand_targets)

			#$brand_targets = "1,2,3,4";
    ...
    // {
        ...
        // $this->db
            // ->select("sale.id, sale.date as date, sales.total, sale.created_by")
