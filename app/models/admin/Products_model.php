...
class Products_model extends CI_Model
{
    ...
    public function addProduct($data, $items, $warehouse_qty, $product_attributes, $photos ,$wholesales)
    {
		...
		if($wholesales) {
                foreach($wholesales as $wholesale) {
                    ...
                    $this->db->insert('product_wholesales', $wholesale);
                }
            }
    ...
    public function updateProduct($id, $data, $items, $warehouse_qty, $product_attributes, $photos, $update_variants, $wholesales)
    {
        if ($this->db->update('products', $data, array('id' => $id))) {
		
		  if($wholesales) {
                foreach($wholesales as $whole) {
                    $whole['product_id'] = $id;
                    if(isset($whole['id'])) {
                        $this->db->update('product_wholesales', $whole, array('id' => $whole['id']));
                    } else {
                        $this->db->insert('product_wholesales', $whole);
                    }
                }
            }
	...
	public function getProductWholesales($product_id)
    {
        $q = $this->db->get_where("product_wholesales", array('product_id' => $product_id));
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
	...
	public function getProductParentStock($product_id,$warehouse_id)
    {
