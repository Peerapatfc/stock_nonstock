...
class Auth_model extends CI_Model
{
    ...
    public function user_privilege($id = null)
    {
       $user_id = !is_null($id) ? $id : $this->session->userdata('user_id');
        
        $this->db->select('*,users.seller_id as seller');
        $this->db->where('users.id',$user_id);
        $this->db->join('privilege','privilege.id = users.privilege_id','INNER');
        $this->db->join('companies', 'companies.email = users.email','INNER');
        $q = $this->db->get('users');
        //print_r($user_id);
        // $q = $this->db->get_where('users', array( 'users.id' => $user_id));
        if ($q->num_rows() > 0) {
    ...
    public function user_privilege2($id = null)
    {
       $user_id = !is_null($id) ? $id : $this->session->userdata('user_id');
    ...
    public function delete_user($id,$Email)
    {
		...
		$this->db->delete('companies', array('email' => $Email, 'user_id' => 'Administrator', 'group_name' => 'user'));
        $this->db->delete('companies', array('email' => $Email, 'user_id' => 'Administrator', 'group_name' => 'biller'));
	...
	public function updateWareHouseUser($warehouse_id,$user_id)
    {
        if ($this->db->update("users", array('warehouse_id' => $warehouse_id), array('id' => $user_id))) {
            return true;
        } else {
            return false;
        }
		...
		public function getPrivilege($order_amount){
		//$q = $this->db->get_where('privilege', array('overselling' => 1));
		//$q = $this->db->get_where('SELECT * FROM sma_privilege WHERE order_amount >= '.$order_amount.' ORDER BY order_amount ASC LIMIT 1');
		
		$this->db->where('order_amount <= '.$order_amount);
		$this->db->order_by('order_amount', 'DESC');
		$this->db->limit(1);
		
		$q = $this->db->get('privilege');
		
		//$q = $this->db->get_where('privilege', array('order_amount >= '.$order_amount), 1);
		
		//$q->order_by('order_amount DESC');
	...
	/*	$q = $this->db->get_where('privilege', array('order_amount >=' ,$order_amount), 1);*/
		...
		//$this->sma->print_arrays($order_amount);
	}
	
	public function getPrivilegeById($privilege_id){
		$q = $this->db->get_where("privilege", array('id' => $privilege_id), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
	...
	public function updatePrivilege($user_id,$privilege_id)
    {
        if ($this->db->update("users", array('privilege_id' => $privilege_id), array('id' => $user_id))) {
            return true;
        } else {
            return false;
        }
	...
	public function getawardpointsrule($id)
    {
        $q = $this->db->get_where('award_points_rule', array('id' => $id));
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
    ...
    {
        if ($this->db->insert("award_points_rule", $data)) {
            return true;
        } else {
            return false;
        }
    ...
    public function updatePointrule($data)
    {
		if ($this->db->update('award_points_rule', $data, array('id' => $data['id']))) {
            return true;
        } else {
            return false;
        }
    ...
    public function getPointrule($id)
    {
        $q = $this->db->get_where("award_points_rule", array('id' => $id), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
	...
	public function exchangepoints($data, $total_points)
    {
		if ($this->db->insert('award_points', $data)) {
			$this->sma->update_award_points($total_points, "", $data['user_id'], "1");
            return true;
        } else {
            return false;
        }
	...
	public function returnpoints($data, $total_points)
    {
		if ($this->sma->update_award_points($total_points, "", $data->user_id, "1") && $data->approve != 0) {
            return true;
        } else {
            return false;
        }
    ...
    public function totalpoint($user_id)
    {
        ...
        $this->db->select('points_current, spent_points, date_insertion, approve, level');
		$this->db->where('user_id', $user_id);
		$this->db->where('status', '1');
		//expiry
		//$this->db->where('date_insertion !=', '0000-00-00');
		//$this->db->where('date_insertion >', $date);

		$q = $this->db->get('award_points');
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
			...
			$count = sizeof($data);
			for($i=0; $i<$count; $i++):
				if($data[$i]->approve == 1 || strlen(trim($data[$i]->approve)) <= 0){
					$points_current = $points_current + $data[$i]->points_current;
					#if($data[$i]->date_insertion < $dateCurStart){
						$spent_points = $spent_points + $data[$i]->spent_points;
					#}
				}
			endfor;
			return $points_current-$spent_points;
        }
        return FALSE;
    }

	public function usepoint($user_id, $level = null)
    {
		$dateCurStart = date("Y-m-d 00:00:00");
		$dateCurEnd = date("Y-m-d 23:59:59");

        $this->db->select('points_current, spent_points, approve, level');
		$this->db->where('user_id', $user_id);
		$this->db->where('status', '1');
		$this->db->where('level', $level);
		$this->db->where('date_insertion >=', $dateCurStart);
		$this->db->where('date_insertion <=', $dateCurEnd);
		$q = $this->db->get('award_points');
        ...
        if ($q->num_rows() > 0) {
			...
			$count = sizeof($data);
			for($i=0; $i<$count; $i++):
				if($data[$i]->approve != 0 || strlen(trim($data[$i]->approve)) <= 0){
					$points = $points + $data[$i]->spent_points;
				}
			endfor;
			return $points;
        }
	...
	public function usepointold($user_id, $level = null)
    {
		$dateCurStart = date("Y-m-d 00:00:00");
	...
	public function getawardpointsById($id)
    {
        $q = $this->db->get_where('award_points', array('id' => $id));
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
    ...
    {
		if ($this->db->update('award_points', $data, array('id' => $id))) {
            return true;
        } else {
            return false;
        }
    ...
    public function deletePointrule($id)
    {
        if ($this->db->delete("award_points_rule", array('id' => $id))) {
            return true;
        }
        return FALSE;
    }
    ...
    {
		if ($this->db->update('users', $data, array('id' => $data['id']))) {
            return true;
        }
        return FALSE;
    }
	...
	public function update_wallet($data)
    {
		...
		if($data['wallet_type']=="deposit"){
			$amount = $data['amount'];
		}else if($data['wallet_type']=="withdraw"){
			$amount = "-".$data['amount'];	
		}*/
	
		$now = date('Y-m-d H:i:s');
		 if ($this->db->insert("wallets", array('user_id' => $data['user_id'], 'amount' => $data['wallet_amount'], 'type' => $data['wallet_type'], 'approve' => "0",'upload_slip' => $data['upload_slip'],'date_wallet' => $now))) {
            return true;
        } else {
            return false;
        }
	...
	public function getWallet() {
		...
		$q = $this->db->get_where('wallets', array('id' => $id));
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
	...
	public function getWalletSlip($id) {
		
		$this->db->select('upload_slip');
		$q = $this->db->get_where('wallets', array('id' => $id));
	//	print_r($q->row());
        if ($q->num_rows() > 0) {
            return $q->row();
	...
	public function approveWallet($data) {
		if ($this->db->update('wallets', $data, array('id' => $data['id']))) {
			return true;
        }
		return FALSE;
	}
	
	public function approveWalletByAdmin($id,$data) {
		if ($this->db->update('wallets', $data, array('id' => $id))) {
			return true;
        }
		return FALSE;
	}
	...
	//profile/seller_id
	public function getProfilebyPhone($seller_id) {
        $this->db->select('companies.*, users.avatar, users.parent_id as parent_id, users.privilege_id as privilege_id, users.team as team, users.img as img, users.user_type as user_type,  users.default_dealer as default_dealer')
            ->join('users', 'users.email = companies.email', 'left');
        $q = $this->db->get_where('companies', array('users.seller_id' => $seller_id));
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
	...
	public function unsetDefaultDealer($id){
		if ($this->db->update('users', array('default_dealer' => 0) , array('id' => $id))) {
			return TRUE;
		}
		  return FALSE;
	}
	
	public function setDefaultDealer($id){
		if ($this->db->update('users', array('default_dealer' => 1) , array('id' => $id))) {
			return TRUE;
		}
		  return FALSE;
	}
	...
	public function checkDefaultDealer($parent_id)
    {
        $q = $this->db->get_where("users", array('parent_id' => $parent_id , 'default_dealer' => 1), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
	...
	public function getParentBySellerId($seller_id)
    {
        $q = $this->db->get_where("users", array('seller_id' => $seller_id), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
	...
	public function getParentBySeller($id)
    {
        $q = $this->db->get_where("users", array('id' => $id), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
	...
	public function upline_data02($term, $limit)
    {
        // $this->db->select("id, (CASE WHEN seller_id = '-' THEN first_name ELSE CONCAT(first_name, ' (', last_name, ')') END) as text", FALSE);
		// $this->db->where(" (id LIKE '%" . $term . "%' OR seller_id LIKE '%" . $term . "%' OR first_name LIKE '%" . $term . "%' OR last_name LIKE '%" . $term . "%') ");
		$q = $this->db->get_where('users', array('active' => '1'), $limit);
		if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
    ...
    public function upline_data($term, $limit = 10)
    {
        $this->db->select("id, (CASE WHEN seller_id = '-' THEN first_name ELSE CONCAT(seller_id, ' (', first_name, ' ',last_name,')') END) as text", FALSE);
		$this->db->where(" (id LIKE '%" . $term . "%' OR first_name LIKE '%" . $term . "%' OR seller_id LIKE '%" . $term . "%' OR email LIKE '%" . $term . "%' OR last_name LIKE '%" . $term . "%') ");
	...
	public function user_seller($id = null) 
    { 
        $user_id = !is_null($id) ? $id : $this->session->userdata('user_id'); 
    ...
    public function last_parant($id) //ตรวจสอบทีมเดิม
    {
        $this->db->select('*');
        $this->db->from('users');
        $this->db->where('id',$id);
