...
class Settings_model extends CI_Model
{
    ...
    {
        if ($this->db->delete("commission_matrixrate", array('id' => $id))) {
            return true;
        }
        return FALSE;
    }
	
    public function commissionadded($data)
    {
        if ($this->db->insert("commission_matrixrate", $data)) {
            return true;
        }
        return false;
    }

    public function updateCommission($id, $data = array())
    {
        if ($this->db->update("commission_matrixrate", $data, array('id' => $id))) {
            return true;
        }
        return false;
    }
    ...
    {
        $q = $this->db->get_where('commission_matrixrate', array('id' => $id), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
