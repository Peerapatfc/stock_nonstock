...
class Reports_model extends CI_Model
{
    ...
    public function getStaff()
    {
        ...
        $this->db->where('group_id !=', 3)->where('group_id !=', 4)->where('active', 1);
        $q = $this->db->get('users');
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
	...
	//reports/orderprovince
	
    public function getZipcodebyProvince($province)
    {
			...
			$this->db
                ->select("name_in_thai, zip_code", FALSE)
				->join($wp, 'FWP.zip_code = sales.zipcode', 'left')
				->where('name_in_thai', $province)
				->group_by('name_in_thai');
    ...
    public function getOrderProvinceByzipcode($zip_code)
    {
        $this->db->select("addr_subdistricts.zip_code, addr_provinces.province_lat, addr_provinces.province_lng, addr_provinces.name_in_thai")
            ->join('addr_districts', 'addr_districts.id = addr_subdistricts.district_id', 'left')
            ->join('addr_provinces', 'addr_provinces.id = addr_districts.province_id', 'left')
			->where('addr_subdistricts.zip_code', $zip_code)->group_by('addr_provinces.name_in_thai');
			
        $q = $this->db->get('addr_subdistricts');
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                //$data[] = $row;
    ...
    public function getSalesByZipcode($zip_code = NULL, $start_date= NULL, $end_date= NULL)
    {
        $this->db
            ->select("count(id) as count_order, grand_total, zipcode")->select_sum('grand_total')
            ->group_by('zipcode')->order_by('sum(grand_total)', 'desc')->limit(10);
		...
		if ($start_date || $end_date) {
         ...
         if ($zip_code) {
             $this->db->where('sales.zipcode', $zip_code);
         }
        $q = $this->db->get('sales');
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
    ...
    {
        $this->db->select("addr_provinces.*");
        $q = $this->db->get('addr_provinces');
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
