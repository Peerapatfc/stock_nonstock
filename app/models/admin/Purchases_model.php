...
class Purchases_model extends CI_Model
{
    ...
    public function getAllPurchaseItems($purchase_id)
    {
        ...
        if ($q->num_rows() > 0) {
			...
			#$data['sum_quantity']  = $item_quantity;
	 ...
	 public function getPurchaseItemsById($purchase_id)
    {
        $q = $this->db->get_where('purchase_items', array('purchase_id' => $purchase_id));
		$item_quantity = 0;
        if ($q->num_rows() > 0) {
	...
	public function getPurchaseByRef($reference_no)
    {
        $q = $this->db->get_where('purchases', array('reference_no' => $reference_no), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
	...
	public function getDeliveryByPurchaseRef($RefNo)
    {
        $q = $this->db->get_where('deliveries', array('sale_reference_no' => $RefNo), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
    ...
    public function updatePrint($id, $is_print, $updated_at)
    {
        if ($this->db->update('purchases', array('is_print' => $is_print, 'updated_at' => $updated_at), array('id' => $id))) {
            return true;
        }
        return false;
    }
    ...
    public function getattachmentByID($id, $return_id = NULL)
    {
	...
	// ใบเสร็จ
    public function updatePurchasesSetting($data)
    {
		if ($this->db->update('purchases_settings', $data, array('id' => 0))) {
            return TRUE;
		}
        return FALSE;
    }
	
    public function getPurchasesSetting()
    {
        $q = $this->db->get_where('purchases_settings', array('id' => 0), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
    ...
    public function getPurchasesReceipt($id, $type)
    {
        $q = $this->db->get_where('purchases_receipt', array('sale_id' => $id, 'type' =>$type), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
    ...
    public function addPurchasesReceipt($data)
    {
        if ($this->db->insert('purchases_receipt',$data)) {
            return TRUE;
        }else{
			return FALSE;
		}
    ...
    public function chkPurchases_Receipt($invoice_no)
    {
        $q = $this->db->get_where('purchases_receipt', array('invoice_no' => $invoice_no), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
    ...
    {
        $q = $this->db->get_where('companies', array('id' => $id, 'group_name' => 'biller'), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
	...
	//	 if ($this->db->insert("award_points", array('reference_no' => $data['reference_no'], 'user_id' => $data['user_id'] , 'date_insertion' => $data['date_insertion'] , 'status' => "0"))) {
            return true;
        } else {
            return false;
        }
	 ...
	 public function update_status_points($data)
	{
	...
	public function checkParentWarehouse($user_id){
		$q = $this->db->get_where('users', array('parent_id' => $user_id), 1);
		if ($q->num_rows() > 0) {
			return $q->row();
		}
		 return FALSE;
	...
	public function adjustParentStock($data,$warehouse_id)
	{
		foreach($data as $dt){
			$wp_details = $this->getWarehouseProductQuantity($warehouse_id, $dt->product_id);
			$quantity_balance =  $wp_details->quantity - $dt->quantity_balance;
			$this->db->update('warehouses_products', array('quantity' => $quantity_balance), array('product_id' => $dt->product_id,'warehouse_id' => $warehouse_id));
			$this->db->update('warehouses_products_variants', array('quantity' => $quantity_balance), array('product_id' => $dt->product_id,'warehouse_id' => $warehouse_id));
			//$this->db->update('purchase_items', array('quantity_balance' => $quantity_balance), array('product_id' => $dt->product_id,'warehouse_id' => $warehouse_id));
		}
		return true;
    ...
    public function isship($id, $data)
    {
		if ($this->db->update('purchases', array('is_ship' => $data['is_ship']), array('id' => $id))) {
            return true;
        } else {
            return false;
        }
	...
	public function additional_document($purchase_id,$data = array())
	{
	 if ($this->db->insert("purchases_additional_document", array('purchase_id' => $purchase_id, 'document' => $data['attachment'] ))) {
			return true;
		} else {
			return false;
		}
	 ...
	 public function get_additional_document($purchase_id)
    {
        $q = $this->db->get_where('purchases_additional_document', array('purchase_id' => $purchase_id));
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
