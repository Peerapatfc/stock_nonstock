 ...
 //          if(chkWidth >800 ||chkHeight >600){
          	
 //          	alert("ขนาดรูปภาพของคุณเกินขนาดที่กำหนดไว้กรุณาเลือกใหม่!!"); 
 // }
		  ...
		  if(imgDataFix == "1"){  // === fix site image
           ...
           $('.img-wait-process').html('<img src="http://127.0.0.1/new-agent-card/assets/images/ajax-loader.gif">');  
           ...
           setTimeout(function(){  
              $.ajax(urlUpload, {
                ...
                success: function (dataRes) {
                    console.log(dataRes.result+'5555');
                    $('#'+imgDataViewReturn).attr('src', 'http://127.0.0.1/new-agent-card/'+ dataRes.result);
                    $('#'+imgDataViewReturn).attr('style','max-width:100px;');
                    $('#imgDataView2').val('http://127.0.0.1/new-agent-card/'+ dataRes.result);
	  ...
	  /*  ====== function crop ===== */
	   function __myFnCrop(paramW, paramH){  
			  ...
			  */
			  ...
			  // Import image
			  var $inputImage_1 = $('#inputImage_1');
			  if (URL) {
				 $inputImage_1.change(function () {
				var filesize = this.files[0].size/1024/1024; // in MB
				 
				 if (filesize > 4){
				 	 alert('ขนาดรูปภาพของคุณเกินที่กำหนด 4 MB กรุณาเลือกใหม่');
				 	 	//window.location.replace("http://127.0.0.1/new-agent-card/");
				 	 	//window.setTimeout(function(){ window.location = "http://www.yoururl.com"; },3000);
				 	// window.location.href='http://127.0.0.1/new-agent-card/';
				 	 end();	
       			}
				...
				$inputImage_1.prop('disabled', true).parent().addClass('disabled');
			 }
