...
//generate pdf and force to download
    ...
    public function pdf($purchase_id = null, $view = null, $save_bufffer = null)
    {
		...
		$PurchasesReceipt = $this->purchases_model->getPurchasesReceipt($id);
        // if (!$this->session->userdata('view_right')) {
            // $this->sma->view_rights($inv->created_by);
        // }
		...
		if(isset($PurchasesReceipt->invoice_no)){
			...
			$sequence_no = sprintf("%0".$purchases_setting->digits."d", $purchases_setting->current);

			$invoice_no = $purchases_setting->prefix.''.date("Ym", strtotime($inv->date)).$sequence_no;
			...
			$current_no = [
				'current' => $purchases_setting->current + 1,
			];
        ...
        #foreach($query->result() as $key=>$row){
            ...
            foreach($sale_items as  $sale_item){
                $order_data[$i++] = [
					...
					'quantity' => intval($sale_item->quantity_balance),
			...
			$vat = $inv->grand_total*7/107;
			$summary = [
				//'cod'		=>	"",
				'shipping'	=>	 number_format($inv->shipping,2),
				'discount'	=>	 number_format($inv->total_discount,2),
				'vat_type'	=>	"include",
				'total'		=>	 number_format($inv->grand_total-$vat,2),
				'vat'		=>	 number_format($vat,2),
				'net_total'	=>	 number_format($inv->grand_total,2),
			];
			$bank_data = [
				 'bank' => $bank->bank,
